...
// 2. 渲染时会统一应用 cardScale 进行整体缩放
// 3. 这确保了卡片作为一个整体，所有元素（背景、图标、文字）按比例缩放
//
// 这些配置不应该暴露给外部调用者，由 PlantCardFactory 和 PlantCardRenderSystem 内部使用
const (
	...
	// 渲染时会乘以 cardScale，确保字体大小随卡片整体缩放
