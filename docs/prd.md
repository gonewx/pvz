...
**史诗目标:** 完善核心游戏体验，实现暂停菜单、植物攻击动画、粒子特效、除草车防线系统和音效系统，提升游戏的完整性和可玩性，确保所有核心机制符合原版游戏标准。

### Epic Overview (史诗概览)

Epic 10 是对核心游戏体验的最后完善，实现后游戏基本达到 MVP 标准。本 Epic 包含 **12 个用户故事**（原计划 4 个 + 扩展 4 个 + 音效系统 4 个），覆盖暂停菜单、除草车系统、攻击动画、粒子特效、阴影渲染、交互反馈和音效系统等核心功能。
...
7. **植物和僵尸阴影渲染** (✅ Done) - 视觉完整性增强
   - 所有植物和僵尸脚下渲染椭圆形阴影
   - 阴影大小根据实体类型自适应
   - 透明度 60-70%, 渲染在实体下方

8. **阳光不足交互反馈** (✅ Done) - 用户体验优化
   - 点击阳光不足卡片时阳光计数器数字闪烁
...
9. **音效系统集成** (📝 Draft) - 核心游戏音效
   - 关卡胜利/失败音效、波次警告音效
   - 割草机音效、僵尸核心音效
   - 音效路径统一修正
   - 详见 `docs/stories/10.9.sound-effect-integration.story.md`

10. **戴夫语音系统** (📝 Planned) - 对话系统集成
    - 疯狂戴夫对话语音
    - 短语、长语、超长语音效支持

11. **完整植物音效系统** (📝 Planned) - 植物战斗体验
    - 窝瓜、火爆辣椒、玉米投手等植物音效
    - 黄油定身效果音效

12. **特殊僵尸音效系统** (📝 Planned) - 僵尸战斗体验
    - 报纸僵尸、撑杆跳僵尸、小丑僵尸、舞王僵尸等
    - 巨人僵尸脚步/砸地音效

### Stories (故事列表)

Epic 10 包含 **12 个 Story**，按优先级排序：
...
**史诗目标:** 为《植物大战僵尸》复刻版实现战斗中存档/读档功能。玩家点击暂停菜单的"主菜单"按钮时自动保存当前战斗进度，再次进入冒险模式时可选择继续游戏或重玩关卡，提供简洁的用户交互体验。

### Epic Overview (史诗概览)

Epic 18 是对现有 SaveManager 系统的扩展，为战斗场景增加完整的存档/读档功能。

**当前系统现状:**
- 现有 `SaveManager` (Story 8.6) 实现了关卡进度保存
- 存储格式: YAML
...
**Acceptance Criteria:**
1. **BattleSaveData 数据结构**: 定义完整的战斗状态数据结构，包含版本号、保存时间、关卡状态、所有实体数据
2. **实体数据序列化**:
   ...
   - 阳光: 位置、生命周期状态
   - 除草车: 所在行、是否已触发
3. **BattleSerializer 实现**: `SaveBattle(em, gameState, filePath) error`, `LoadBattle(filePath) (*BattleSaveData, error)`
4. **SaveManager 扩展**: `HasBattleSave()`, `GetBattleSaveInfo()`, `DeleteBattleSave()`, `GetBattleSavePath()`
5. **单元测试**: 覆盖率 ≥ 80%
...
**Acceptance Criteria:**
1. **暂停菜单"主菜单"按钮触发保存**: 修改 `PauseMenuModule.onMainMenu` 回调，调用 `BattleSerializer.SaveBattle()` 保存当前状态
2. **冒险模式入口检测存档**: 修改 `MainMenuScene.onStartAdventureClicked()`，检查 `SaveManager.HasBattleSave(username)`
3. **存档信息读取**: 实现 `BattleSaveInfo` 结构（关卡ID、保存时间）用于对话框显示
4. **日志输出**: 记录保存/加载操作，支持调试模式显示详细信息
5. **单元测试**: 覆盖率 ≥ 80%
...
**Acceptance Criteria:**
1. **继续游戏对话框**: Title: "继续游戏?", Message: "你想继续当前游戏还是重玩此关卡？"
   - 按钮布局（两行）:
     - 第一行: [继续] [重玩关卡]
     - 第二行: [取消]
...
2. **按钮行为**:
   ...
   - **重玩关卡**: 删除存档，正常开始该关卡
   - **取消**: 关闭对话框，返回主菜单
3. **GameScene.LoadFromSave() 实现**: 恢复 GameState 状态、重建所有实体、恢复草坪网格状态
4. **动画状态恢复**: Reanim 动画从 idle 状态开始（简化处理）
5. **错误处理**: 存档损坏时提示用户，提供删除损坏存档的选项
6. **单元测试**: 覆盖率 ≥ 80%
...
- **Epic 18 完整规范**: `docs/prd/epic-18-battle-save-system.md`
