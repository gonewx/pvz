...
2. **Story 8.2**: 关卡 1-1 教学引导系统 ✅
   - **Story 8.2.1**: 教学关卡视觉提示增强（Hotfix - 补充 8.2 缺失的视觉反馈）📝
...
8. **Story 8.8**: 僵尸获胜流程与游戏结束对话框 ✅ (需代码修复 Phase 2 - 改为同时执行)
...
Epic 10 是对核心游戏体验的最后完善，实现后游戏基本达到 MVP 标准。本 Epic 包含 **8 个用户故事**（原计划 4 个 + 扩展 4 个），覆盖暂停菜单、除草车系统、攻击动画、粒子特效、阴影渲染和交互反馈等核心功能。

**核心功能模块:**

#### 原计划模块 (Story 10.1-10.4)

1. **暂停菜单系统** (✅ Done) - 用户体验核心功能
   - 暂停/恢复游戏
   - 菜单面板（继续、重新开始、返回主菜单）
   - 音乐控制
   - ESC 键快捷键

2. **除草车最后防线系统** (✅ Done) - 原版标志性机制
   - 每行左侧台阶放置除草车
   - 僵尸到达左侧触发
   - 除草车自动清除所在行所有僵尸
   - 一次性使用，失败条件增强

3. **植物攻击动画系统** (✅ Done) - 视觉表现增强
   - 植物发射子弹时切换攻击动画
   - 动画状态机管理
   - 支持多种射手植物
   - 使用原版 Reanim 动画

4. **植物种植粒子特效** (✅ Done) - 视觉反馈完善
   - 种植时土粒飞溅效果
   - 使用原版粒子配置
   - 抛物线运动
   - 性能优化

#### 扩展模块 (Story 10.5-10.8)

5. **植物攻击动画帧事件同步** (✅ Done) - 动画精确控制
   - 子弹在攻击动画关键帧发射
   - 配置关键帧实现零延迟同步
   - 子弹起始位置使用实时轨道坐标

6. **除草车碾压僵尸压扁动画** (✅ Done) - 死亡动画增强
   - 僵尸被除草车碾压时播放压扁动画
   - 使用 LawnMoweredZombie.reanim 变形数据
   - 压扁动画结束后触发粒子效果
...
8. **阳光不足交互反馈** - 用户体验优化
   - 点击阳光不足卡片时阳光计数器数字闪烁
   - 红/黑闪烁 (0.3秒周期, 持续 1.0秒)
   - 播放无效操作音效

### Stories (故事列表)

Epic 10 包含 **8 个 Story**，按优先级排序：
...
#### **Story 10.5: 植物攻击动画帧事件同步** (✅ Done)
> **As a** 玩家,
> **I want** 子弹在攻击动画的关键帧发射,
> **so that** 攻击动画和子弹发射完美同步，视觉表现自然流畅。

**状态**: ✅ 已完成 (Sprint Change Proposal - 2025-10-27)
**优先级**: ⭐⭐⭐⭐ 中高
**工作量**: 6-9 小时
**文档**: `docs/stories/10.5.story.md`

---

#### **Story 10.6: 除草车碾压僵尸压扁动画** (✅ Done)
> **As a** 玩家,
> **I want** 僵尸被除草车碾压时播放压扁动画,
> **so that** 死亡效果更加生动有趣，符合原版游戏表现。

**状态**: ✅ 已完成 (Sprint Change Proposal - 2025-11-20)
**优先级**: ⭐⭐⭐ 中
**工作量**: 9-15 小时
**文档**: `docs/stories/10.6.story.md`

---

#### **Story 10.7: 植物和僵尸阴影渲染** (📝 Pending)
> **As a** 玩家,
> **I want** 看到植物和僵尸脚下的阴影,
> **so that** 游戏场景更有深度感和真实感，符合原版游戏的视觉表现。

**状态**: 📝 待实现 (Sprint Change Proposal - 2025-11-24)
**优先级**: ⭐⭐⭐⭐ 中高
**工作量**: 6-8 小时
...
1. ✅ 所有 **8 个用户故事**完成，AC 全部满足
2. ✅ 暂停菜单功能完整，UI 美观，交互流畅
3. ✅ 植物攻击动画自然切换，无卡顿或闪烁
4. ✅ 种植粒子特效符合原版表现，性能良好
5. ✅ 除草车系统完整，触发准确，动画流畅
6. ✅ 攻击动画帧事件同步精确，视觉表现完美
7. ✅ 除草车压扁动画效果生动，符合原版表现
8. ✅ 植物和僵尸阴影渲染自然，增强场景深度感
9. ✅ 阳光不足反馈明确，交互体验友好
10. ✅ 现有功能无回归，通过 QA 测试
11. ✅ 代码符合编码规范，通过 linter 检查
12. ✅ 所有新功能有对应的单元测试（覆盖率 > 80%）
13. ✅ 文档更新（AGENTS.md, CLAUDE.md）
14. ✅ 游戏体验达到 MVP 标准
...
- `ShadowComponent` - 实体阴影渲染参数
- `SunCounterFlashComponent` - 阳光计数器闪烁状态

**新增系统:**
- `PauseMenuRenderSystem` - 暂停菜单渲染
- `LawnmowerSystem` - 除草车触发和移动管理
- `ShadowRenderSystem` - 阴影批量渲染

**系统修改:**
- `GameScene.Update()` - 暂停状态检测
- `InputSystem.handleLawnClick()` - 种植粒子效果触发
- `InputSystem.handlePlantCardClick()` - 阳光不足反馈触发
- `BehaviorSystem` - 植物攻击动画管理和帧事件同步
- `LevelSystem.checkDefeatCondition()` - 除草车状态检测
- `RenderSystem` - 阴影渲染集成
- `UIRenderSystem` - 阳光计数器闪烁渲染
...
| Story 10.7: 阴影渲染 | 6-8 小时 | 中高 | 📝 Pending |
| Story 10.8: 阳光反馈 | 4-6 小时 | 中 | 📝 Pending |
| **总计** | **55-80 小时** | - | 6/8 Done |

**已完成工作量**: 45-66 小时 (Story 10.1, 10.2, 10.3, 10.4, 10.5, 10.6)
**剩余工作量**: 10-14 小时 (Story 10.7, 10.8)

**建议实施顺序 (剩余 Story):**
1. Story 10.7（阴影渲染）- 视觉完整性重要，中高优先级
2. Story 10.8（阳光反馈）- 交互体验优化
...
- **Story 10.5 详细文档**: `docs/stories/10.5.story.md`
- **Story 10.6 详细文档**: `docs/stories/10.6.story.md`
- **Story 10.7 详细文档**: `docs/stories/10.7-plant-zombie-shadow-rendering.md`
- **Story 10.8 详细文档**: `docs/stories/10.8-sun-shortage-feedback.md`
...
**优先级**: High（完成 MVP 必需）
**预估总工作量**: 55-80 小时（原计划 30-42 小时 + 扩展 25-38 小时）
**最后更新**: 2025-11-24 (Sprint Change Proposal - 新增 Story 10.7, 10.8)

---

## **Epic 17: 僵尸生成引擎 (Zombie Generation Engine)**

**史诗目标:** 实现《植物大战僵尸》冒险模式的精确僵尸生成系统，包括难度动态调整、行分配平滑权重算法、波次计时控制和旗帜波特殊机制，完全还原原版游戏的出怪逻辑和游戏节奏。

### Epic Overview (史诗概览)

Epic 17 将实现原版 PvZ 冒险模式的核心僵尸生成机制，基于逆向工程分析的精确算法：
...
1. **Story 17.1**: 难度引擎 - 轮数与级别容量计算
2. **Story 17.2**: 关卡脚本格式升级与解析器
3. **Story 17.3**: 僵尸生成限制检查系统
4. **Story 17.4**: 平滑权重行分配算法 - 核心实现
5. **Story 17.5**: 合法行判定系统
6. **Story 17.6**: 波次刷新计时器 - 常规波次
7. **Story 17.7**: 旗帜波特殊计时与红字警告
8. **Story 17.8**: 加速刷新机制 - 血量触发
9. **Story 17.9**: 僵尸物理与边界 - 坐标系统
10. **Story 17.10**: 集成测试与关卡验证

### Success Criteria (成功标准)

1. ✅ 难度引擎正确计算轮数和级别容量
2. ✅ 平滑权重行分配算法实现，分布自然
3. ✅ 波次计时系统完整，包括旗帜波和最终波
4. ✅ 加速刷新机制正确触发
5. ✅ 僵尸出生坐标和进家判定准确
6. ✅ 关卡 1-1 至 1-4 验证通过
7. ✅ 单元测试覆盖率 ≥ 80%
...
| Story | 预估工作量 | 优先级 |
|-------|-----------|--------|
| Story 17.1-17.3 | 14-20 小时 | 高 |
| Story 17.4-17.5 | 12-18 小时 | 高 |
| Story 17.6-17.8 | 20-26 小时 | 高 |
| Story 17.9-17.10 | 10-14 小时 | 中 |
| **总计** | **56-78 小时** | - |

**详细设计文档**: 完整的 Epic 17 规范请参见 [Epic 17 详细文档](prd/epic-17-zombie-generation-engine.md)。

### Reference Documentation (参考文档)

- **设计文档**: `.meta/levels/ZombieGenerationModuleDesignDocument.md`
- **Epic 17 完整规范**: `docs/prd/epic-17-zombie-generation-engine.md`
