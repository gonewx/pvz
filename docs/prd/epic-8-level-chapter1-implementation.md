...
### Story 8.7: 特殊关卡类型系统 - 传送带模式(1-5, 1-10)
**目标**: 实现传送带模式特殊玩法机制，包括坚果保龄球(1-5)和随机植物传送带(1-10)
...
### Story 8.9: 关卡 1-6 实现 - 寒冰射手与撑杆僵尸
**目标**: 实现第一章关卡 1-6，引入寒冰射手（控制类植物）和撑杆僵尸（特殊能力僵尸）
...
**目标**: 实现关卡 1-7 配置，解锁大嘴花作为通关奖励

**范围**:
- 关卡 1-7 配置（5行草地、2面旗帜、20波僵尸）
- 大嘴花作为通关奖励解锁
- 奖励动画和介绍面板

**验收标准**:
- 能够从关卡选择界面进入关卡 1-7
- 关卡配置正确：5行草地、2面旗帜、20波僵尸
- 关卡完成后显示奖励动画和大嘴花介绍面板
- 完成关卡后大嘴花被解锁

> **注意**: 大嘴花行为实现归属 Story 8.12，因为玩家在 1-7 中不能使用大嘴花（它是通关奖励）

**状态**: ✅ Done

**参考**: `docs/stories/8.11.story.md`

---

### Story 8.12: 关卡 1-8 实现 - 选卡系统与大嘴花行为
**目标**: 实现关卡 1-8，引入选卡系统、双发射手、铁桶僵尸，以及大嘴花首次可用

**范围**:
- **选卡系统** ⭐（游戏循环重大变更）
  - 新流程：镜头预览 → 选卡界面 → "一起摇滚吧!" → 开始游戏
  - 顶部卡槽 + 植物仓库（7张卡片）+ 开始按钮
  - 选择/取消植物交互、卡槽满启用按钮
  - 僵尸预览功能（右侧显示僵尸阵容）
- **双发射手 (Repeater) 实现**
...
- 铁桶僵尸正确显示和行为
- 大嘴花正确执行吞噬/撕咬/消化机制
- 关卡完成后奖励动画和双发射手介绍面板显示正确

**状态**: 📝 Draft

**参考**: `.meta/levels/chapter1.md` (Level 1-8 选卡系统)

---

### Story 8.13: 奖励动画系统行为优化与主菜单按钮
**目标**: 优化奖励动画中卡包等待阶段的交互行为，并在奖励面板上添加主菜单按钮

**范围**:
- **卡包等待阶段交互优化**
  - waiting 阶段只响应卡包点击或空格键（不再是任意点击）
  - 界面其他元素保持可操作状态
  - 鼠标悬停卡包时显示手形光标
- **主菜单按钮功能**
  - 奖励面板右上角显示主菜单按钮
  ...
  - 点击后返回主菜单
- **配置支持**
  - `LevelConfig` 新增 `showMainMenuButton` 字段（默认 true）
  - 1-1、1-2 关卡设置为 false（教学关卡不显示）

**验收标准**:
- waiting 阶段只有点击卡包或按空格键才进入展开阶段
- 鼠标悬停卡包时切换为手形光标
- 奖励面板正确显示主菜单按钮（除 1-1、1-2 外）
- 点击主菜单按钮后跳转到主菜单
- 单元测试覆盖率 ≥ 80%

**状态**: 📝 Draft

**参考**: `docs/stories/8.13.reward-animation-enhancement.story.md`
...
- [ ] **Story 8.7**: 特殊关卡类型系统 - 传送带模式(1-5, 1-10) - 📝 待创建
- [x] **Story 8.8**: 僵尸获胜流程与游戏结束对话框 - ✅ Done (需代码修复 Phase 2 - 改为同时执行)
- [x] **Story 8.9**: 关卡 1-6 实现 - 寒冰射手与撑杆僵尸 - ✅ Done
- [ ] **Story 8.10**: 第一章集成测试与调优 - 📝 待创建
- [x] **Story 8.11**: 关卡 1-7 实现 - 大嘴花解锁 - ✅ Done
- [ ] **Story 8.12**: 关卡 1-8 实现 - 选卡系统与大嘴花行为 - 📝 Draft
- [ ] **Story 8.13**: 奖励动画系统行为优化与主菜单按钮 - 📝 Draft
...
| 2025-12-11 | 1.2 | Story 8.7 扩展为传送带模式(1-5, 1-10)合并实现；Story 8.9 更新为关卡 1-6 实现(寒冰射手与撑杆僵尸) | Sarah (PO) |
| 2025-12-16 | 1.3 | 新增 Story 8.11: 关卡 1-7 实现 - 大嘴花 (Chomper)，补充关卡清单缺失的 Story 覆盖 | Bob (Scrum Master) |
| 2025-12-16 | 1.4 | **范围修正**：Story 8.11 缩小为仅关卡配置和奖励解锁；新增 Story 8.12（关卡 1-8）包含选卡系统、双发射手、铁桶僵尸和大嘴花行为实现 | Bob (Scrum Master) |
| 2025-12-17 | 1.5 | 新增 Story 8.13：奖励动画系统行为优化与主菜单按钮 | Sarah (PO) |
