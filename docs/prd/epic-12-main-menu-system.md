...
2. **Story 12.3: 对话框系统基础（重新设计 - 先实现）**
   - 创建通用对话框组件（九宫格拉伸）
   - 实现蒙板叠加系统（支持 Alpha 蒙板）
   - 实现帮助面板（便笺背景 + 帮助文本 + 蒙板）
   - 实现修改版选项面板（不显示继续/重启，"返回游戏"改为"确定"）
   - 实现未解锁提示对话框
   - 实现对话框关闭逻辑（确定/ESC/外部点击）
   - 优先级：⭐⭐⭐⭐⭐ 高
   - 估计工作量：10-14 小时

3. **Story 12.2: 底部功能栏集成（重新设计 - 依赖 12.3）**
   - 实现 3 个花瓶样式按钮（Options/Help/Quit）
   - 实现按钮交互（正常态/悬停态切换）
   - 选项按钮 → 调用选项面板工厂（Story 12.3）
   - 帮助按钮 → 调用帮助面板工厂（Story 12.3）
   - 退出按钮 → 退出游戏
   - 添加点击音效
   - 优先级：⭐⭐⭐⭐⭐ 高
   - 估计工作量：2-4 小时
...
**并行依赖:**
- Story 12.1 可独立开发
- **Story 12.3 必须先于 Story 12.2 完成**（Story 12.2 依赖 12.3 的对话框工厂）
...
| Story | 预估工作量 | 状态 | 优先级 |
|-------|-----------|------|--------|
| Story 12.1: 石碑菜单系统完善 | 8-12 小时 | Done | 高 |
| Story 12.3: 对话框系统基础（重新设计） | 10-14 小时 | Done | 高 |
| Story 12.2: 底部功能栏集成（重新设计） | 2-4 小时 | Done | 高 |
| Story 12.4: 用户管理 UI | 8-10 小时 | Done | 中高 |
| Story 12.5: 解锁功能入口 | 6-8 小时 | 待创建 | 中 |
| Story 12.6: Reanim 动画集成 | 6-8 小时 | InProgress（80% 完成） | 中 |
| Story 12.7: 背景动态效果 | 4-6 小时 | Done | 低 |
| **总计** | **44-62 小时** | - | - |

**建议实施顺序:**
1. 阶段 1（必做）- Story 12.1、**12.3**、**12.2** - 20-30 小时
...
### Story 12.6 和 12.7 实施说明

**Story 12.6（Reanim 动画集成）**:
- ✅ 大部分功能在 Story 12.1 和 Epic 13 实现过程中顺带完成
- ✅ AC1（加载 SelectorScreen.reanim）- 已完成
- ✅ AC3（播放石碑升起动画）- 已完成
- ✅ AC4（与 ReanimSystem 集成）- 已完成
- ✅ AC5（动画配置化管理）- 已完成
- ⏸️ **AC2（播放僵尸手掌升起动画）- 未实现**
  - 原因：功能优先级调整，先完成核心菜单交互
  - 资源文件：`data/reanim/Zombie_hand.reanim`（已存在）
  - 图片资源：`assets/reanim/Zombie_hand_*.png`（已存在）
  - 功能说明：**场景过渡动画** - 点击"冒险模式"按钮后播放，播放完成后切换到关卡场景
  - 计划：**在本 Story 中完成实现**（无需创建新 Story）
  - 实现方式：
    - 创建 `data/reanim_config/zombie_hand.yaml` 配置文件
    - 在主菜单中创建僵尸手掌实体（初始隐藏）
    - 修改冒险模式按钮回调，点击后先播放动画，再切换场景
    - 实现动画完成检测（`ReanimComponent.IsAnimationFinished()` 方法）
    - 阻塞式播放：动画期间禁用其他交互
- 当前状态：InProgress（80% 完成）
- 实施路径：部分顺带完成（Story 12.1, Epic 13）

**Story 12.7（背景动态效果）**:
- ✅ 所有功能在 Story 12.1 实现过程中顺带完成
- ✅ AC1（云朵缓慢飘动效果）- 已完成
- ✅ AC2（草丛轻微晃动效果）- 已完成
- ✅ AC3（动画性能优化）- 已完成
- ✅ AC4（与 Reanim 系统集成）- 已完成
- 当前状态：Done（100% 完成）
- 实施路径：完全顺带完成（Story 12.1）

**追溯创建说明**:
- Story 12.6 和 12.7 的文档在 2025-11-18 追溯创建，用于记录已完成的功能
- 这两个 Story 未正式创建独立文档时，部分功能已在其他 Story 实现过程中完成
- 文档创建目的：提高项目可追溯性，清晰记录实施路径
