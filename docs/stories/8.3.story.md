...
## Status
In Progress - Core implementation completed, pending integration testing
...
1. **实现卡片包掉落动画**（2 hours）
   - 关卡完成后，从屏幕右侧掉落新植物卡片包到草坪中央
   - 使用抛物线轨迹动画（类似真实物理的弧线运动）
...
- [ ] 创建 `pkg/components/reward_animation_component.go`：
  - [ ] 定义 `RewardAnimationComponent`：
    ```go
    type RewardAnimationComponent struct {
        Phase           string    // 动画阶段："dropping", "bouncing", "expanding", "showing", "closing"
        ElapsedTime     float64   // 当前阶段已用时间
        StartX, StartY  float64   // 抛物线起点
        TargetX, TargetY float64  // 抛物线终点
        VelocityX, VelocityY float64 // 抛物线速度
        PlantID         string    // 解锁的植物ID（如 "sunflower"）
    }
    ```
  - [ ] 添加 GoDoc 注释
- [ ] 创建 `pkg/components/reward_panel_component.go`：
  - [ ] 定义 `RewardPanelComponent`：
    ```go
    type RewardPanelComponent struct {
        PlantID         string    // 解锁的植物ID
        PlantName       string    // 植物名称（从 LawnStrings 加载）
        PlantDescription string   // 植物描述（从 LawnStrings 加载）
        CardScale       float64   // 卡片缩放比例（动画用）
        CardX, CardY    float64   // 卡片位置
...
- [ ] 创建 `pkg/systems/reward_animation_system.go`：
  - [ ] 定义 `RewardAnimationSystem` 结构体：
    ```go
    type RewardAnimationSystem struct {
        entityManager   *ecs.EntityManager
        gameState       *game.GameState
        resourceManager *game.ResourceManager
        rewardEntity    ecs.EntityID  // 奖励动画实体ID
    }
    ```
  - [ ] 实现 `NewRewardAnimationSystem(em, gs, rm) *RewardAnimationSystem`
  - [ ] 实现 `TriggerReward(plantID string)` 方法：
    - [ ] 创建奖励动画实体
    - [ ] 添加 RewardAnimationComponent（Phase="dropping"）
    - [ ] 添加 PositionComponent（起点：屏幕右侧上方）
    - [ ] 添加 SpriteComponent（卡片包图片）
    - [ ] 触发 Award.xml 粒子特效（跟随卡片包移动）
  - [ ] 实现 `Update(dt float64)` 方法：
    - [ ] Phase "dropping": 抛物线运动逻辑
      - [ ] 更新位置：`X += VelocityX * dt`, `Y += VelocityY * dt`
      - [ ] 应用重力：`VelocityY += gravity * dt`
      - [ ] 检测到达目标（草坪中央）→ 切换到 "bouncing"
    - [ ] Phase "bouncing": 弹跳动画
      - [ ] 播放 3 次小幅弹跳（使用 sin 曲线）
      - [ ] 完成后切换到 "expanding"
    - [ ] Phase "expanding": 卡片展开动画
      - [ ] 等待玩家点击卡片包
      - [ ] 点击后触发展开特效（粒子爆发）
      - [ ] 切换到 "showing"，显示奖励面板
    - [ ] Phase "showing": 显示奖励面板
      - [ ] 创建 RewardPanelComponent 实体
      - [ ] 加载植物信息（名称、描述）
      - [ ] 卡片渐渐放大并移动到上方中央（scale: 0.5 → 1.5）
      - [ ] 等待玩家点击关闭
    - [ ] Phase "closing": 关闭奖励面板
      - [ ] 淡出动画
      - [ ] 清理奖励实体
      - [ ] 触发场景切换（返回主菜单）
  - [ ] 实现辅助方法：
    - [ ] `calculateParabola(t float64) (x, y float64)` - 抛物线轨迹计算
    - [ ] `handleClick(mouseX, mouseY float64)` - 点击检测
  - [ ] 添加 GoDoc 注释

#### Task 3: 实现奖励面板渲染（AC: 2）
- [ ] 修改 `pkg/systems/render_system.go` 或创建 `pkg/systems/reward_panel_render_system.go`：
  - [ ] 在 `Draw()` 方法中添加奖励面板渲染逻辑
  - [ ] 查询 RewardPanelComponent 实体（使用泛型 API）
  - [ ] 绘制奖励背景（AwardScreen_Back.jpg，全屏）
  - [ ] 绘制标题文本："你得到了一株新植物！"
    - [ ] 从 LawnStrings.txt 加载文本键 `[NEW_PLANT]`
    - [ ] 使用 SimHei.ttf 字体，36px，居中对齐
    - [ ] 位置：Y = screenHeight * 0.2
  - [ ] 绘制植物卡片（SeedPacket_Larger.png）
    - [ ] 应用缩放动画（CardScale）
    - [ ] 位置：(CardX, CardY)
  - [ ] 绘制植物名称和描述
    - [ ] 名称：28px，粗体，居中
    - [ ] 描述：20px，普通，居中，Y = screenHeight * 0.7
  - [ ] 绘制提示文本："点击任意位置继续"
    - [ ] 18px，半透明，底部中央
  - [ ] 渲染层级：在所有游戏元素之上

#### Task 4: 与 LevelSystem 集成（AC: 3）
- [ ] 修改 `pkg/systems/level_system.go`：
  - [ ] 在关卡完成检测逻辑中（所有僵尸被击败）：
    - [ ] 检查是否有新植物解锁：`unlockedPlant := gs.GetPlantUnlockManager().GetLastUnlocked()`
    - [ ] 如果有新植物，调用 `rewardSystem.TriggerReward(unlockedPlant)`
    - [ ] 暂停游戏更新（禁用其他系统，只保留 RewardAnimationSystem）
    - [ ] 否则，直接返回主菜单
  - [ ] 添加日志输出：`log.Printf("Triggering reward animation for plant: %s", unlockedPlant)`

#### Task 5: 加载奖励资源（AC: 1, 2）
- [ ] 修改 `assets/config/resources.yaml`：
  - [ ] 添加奖励动画资源到 `loadingimages` 组：
    ```yaml
    - id: IMAGE_AWARD_SCREEN_BACK
      path: images/AwardScreen_Back.jpg
    - id: IMAGE_SEED_PACKET_LARGER
      path: images/SeedPacket_Larger.png
    # 粒子图片（Award.xml 使用）
    - id: IMAGE_AWARD_RAYS
      path: particles/AwardRays.png
    - id: IMAGE_AWARD_GLOW
      path: particles/AwardGlow.png
    ```
  - [ ] 在 `GameScene.NewGameScene()` 中预加载资源

#### Task 6: 更新 PlantUnlockManager（AC: 2, 3）
- [ ] 修改 `pkg/game/plant_unlock_manager.go`：
  - [ ] 添加方法 `GetLastUnlocked() string`：
    - [ ] 返回最后一次解锁的植物ID
    - [ ] 用于奖励动画显示
  - [ ] 添加植物名称和描述数据：
    ```go
    var PlantInfo = map[string]struct {
        NameKey        string  // LawnStrings.txt 键
        DescriptionKey string  // LawnStrings.txt 键
    }{
        "sunflower": {"SUNFLOWER_NAME", "SUNFLOWER_DESC"},
        "cherrybomb": {"CHERRYBOMB_NAME", "CHERRYBOMB_DESC"},
        // ...
    }
    ```
  - [ ] 实现 `GetPlantInfo(plantID string) (name, desc string)`
...
- [ ] 创建 `pkg/components/camera_component.go`：
  - [ ] 定义 `CameraComponent`：
    ```go
...
- [ ] 创建 `pkg/systems/camera_system.go`：
  - [ ] 定义 `CameraSystem` 结构体：
    ```go
    ...
    ```
  - [ ] 实现 `NewCameraSystem(em, gs) *CameraSystem`
    - [ ] 创建镜头实体
    - [ ] 添加 CameraComponent（默认位置 (0, 0)）
  - [ ] 实现 `Update(dt float64)` 方法：
    - [ ] 查询 CameraComponent
    - [ ] 如果 `IsAnimating`：
      - [ ] 计算当前位置到目标位置的距离
      - [ ] 应用 Easing 函数平滑插值
      - [ ] 更新 GameState.CameraX（当前只支持水平移动）
      - [ ] 检查是否到达目标（距离 < 5px）→ `IsAnimating = false`
  - [ ] 实现 `MoveTo(targetX, targetY, speed float64)` 方法：
    - [ ] 设置 CameraComponent 的 TargetX, TargetY, AnimationSpeed
    - [ ] 设置 `IsAnimating = true`
  - [ ] 实现 `StopAnimation()` 方法：
    - [ ] 设置 `IsAnimating = false`
    - [ ] 立即设置镜头到目标位置
  - [ ] 实现 Easing 函数：
    - [ ] `easeInOutQuad(t float64) float64` - 二次缓动
  - [ ] 添加 GoDoc 注释

#### Task 9: 创建开场动画状态组件（AC: 5）
- [ ] 创建 `pkg/components/opening_animation_component.go`：
  - [ ] 定义 `OpeningAnimationComponent`：
    ```go
...
- [ ] 创建 `pkg/systems/opening_animation_system.go`：
  - [ ] 定义 `OpeningAnimationSystem` 结构体：
    ```go
    ...
    ```
  - [ ] 实现 `NewOpeningAnimationSystem(em, gs, rm, levelConfig, cameraSystem) *OpeningAnimationSystem`
    - [ ] 检查是否需要开场动画：
      - [ ] 如果 `levelConfig.SkipOpening == true`：直接返回 nil
      - [ ] 如果 `levelConfig.OpeningType == "tutorial"`：返回 nil
      - [ ] 如果 `levelConfig.SpecialRules != ""`：返回 nil（迷你游戏）
    - [ ] 创建开场动画实体
    - [ ] 添加 OpeningAnimationComponent（State="idle"）
  - [ ] 实现 `Update(dt float64)` 方法：
    - [ ] 查询 OpeningAnimationComponent
    - [ ] 检查快捷键跳过（ESC 或 Space）：
      - [ ] 如果按下，调用 `Skip()` 方法
    - [ ] 状态机处理：
      - [ ] State "idle": 等待 0.5 秒 → 切换到 "cameraMoveRight"
      - [ ] State "cameraMoveRight":
        - [ ] 调用 `cameraSystem.MoveTo(800, 0, 300)` （右移到僵尸预告位置）
        - [ ] 等待镜头动画完成 → 切换到 "showZombies"
      - [ ] State "showZombies":
        - [ ] 生成预告僵尸实体（调用 `spawnPreviewZombies()`）
        - [ ] 等待 2 秒展示僵尸 → 切换到 "cameraMoveLeft"
      - [ ] State "cameraMoveLeft":
        - [ ] 调用 `cameraSystem.MoveTo(0, 0, 300)` （返回草坪）
        - [ ] 等待镜头动画完成 → 切换到 "gameStart"
      - [ ] State "gameStart":
        - [ ] 清理预告僵尸实体（调用 `clearPreviewZombies()`）
        - [ ] 销毁开场动画实体
        - [ ] 启用其他游戏系统（WaveSpawnSystem 等）
  - [ ] 实现 `spawnPreviewZombies()` 方法（AC: 6）：
    - [ ] 从 `levelConfig.Waves` 获取所有僵尸类型
    - [ ] 去重，获取唯一僵尸类型列表
    - [ ] 在屏幕右侧生成僵尸实体（X=1200, Y按行分布）
    - [ ] 设置僵尸行为为 "idle"（不移动，只播放动画）
    - [ ] 保存僵尸实体ID到 `ZombieEntities`
  - [ ] 实现 `clearPreviewZombies()` 方法：
    - [ ] 遍历 `ZombieEntities`，删除所有预告僵尸
    - [ ] 清空 `ZombieEntities` 列表
  - [ ] 实现 `Skip()` 方法（AC: 7）：
    - [ ] 停止镜头动画：`cameraSystem.StopAnimation()`
    - [ ] 清理预告僵尸：`clearPreviewZombies()`
    - [ ] 设置 `IsSkipped = true`
    - [ ] 切换到 "gameStart" 状态
  - [ ] 添加 GoDoc 注释
...
- [ ] 修改 `pkg/scenes/game_scene.go`：
  - [ ] 在 `NewGameScene()` 中创建系统：
    - [ ] 创建 CameraSystem（总是创建）
    - [ ] 创建 OpeningAnimationSystem（条件创建）：
      ```go
      ...
      ```
    - [ ] 如果 `openingSystem != nil`：
      - [ ] 禁用其他游戏系统（WaveSpawnSystem, InputSystem 等）
      - [ ] 等待开场动画完成后再启用
  - [ ] 在 `Update()` 中添加开场动画更新：
    ```go
    ...
    ```
  - [ ] 在 `Draw()` 中正常渲染（镜头已由 CameraSystem 控制）

#### Task 12: 配置系统验证（AC: 8）
- [ ] 修改 `pkg/config/level_config.go`：
  - [ ] 验证 `SkipOpening` 字段已存在（Story 8.1 已添加）
  - [ ] 验证 `OpeningType` 字段已存在（Story 8.1 已添加）
  - [ ] 在 `applyDefaults()` 中确保默认值：
    ```go
    ...
    ```
- [ ] 更新 `data/levels/level-1-1.yaml` 验证：
  - [ ] 确认 `openingType: "tutorial"`（无开场动画）
  - [ ] 确认 `skipOpening: true`（调试用）
...
- [ ] 修改 `pkg/scenes/game_scene.go` 或 `main.go`：
  ...
  - [ ] 添加命令行参数支持：
    ...
    ```
- [ ] 在 InputSystem 中添加快捷键处理：
  - [ ] ESC 或 Space 键触发 `openingSystem.Skip()`

---

### Task 14: 单元测试（AC: 全部）
- [ ] 创建 `pkg/systems/reward_animation_system_test.go`：
  - [ ] `TestRewardAnimationSystem_TriggerReward`: 验证奖励触发
  - [ ] `TestRewardAnimationSystem_ParabolaCalculation`: 验证抛物线计算
  - [ ] `TestRewardAnimationSystem_ClickDetection`: 验证点击检测
- [ ] 创建 `pkg/systems/camera_system_test.go`：
  - [ ] `TestCameraSystem_MoveTo`: 验证镜头移动
  - [ ] `TestCameraSystem_EasingFunction`: 验证缓动函数
  - [ ] `TestCameraSystem_StopAnimation`: 验证停止动画
- [ ] 创建 `pkg/systems/opening_animation_system_test.go`：
  - [ ] `TestOpeningAnimationSystem_StateMachine`: 验证状态机流转
  - [ ] `TestOpeningAnimationSystem_Skip`: 验证跳过功能
  - [ ] `TestOpeningAnimationSystem_ZombiePreview`: 验证僵尸预告生成
...
2. **Phase 2 完成** - 开场动画系统核心功能已实现
   - 创建了 CameraComponent 和 OpeningAnimationComponent
   - 实现了 CameraSystem 镜头控制（平滑移动、停止）
   - 实现了 OpeningAnimationSystem 状态机（idle → cameraMoveRight → showZombies → cameraMoveLeft → gameStart）
   - 支持跳过功能（ESC/Space 键）

3. **集成工作完成**
   - 在 GameScene 中创建 RewardAnimationSystem
   - LevelSystem 构造函数已更新，接受 RewardAnimationSystem 参数
   - 开场动画系统代码已实现但暂时未启用（需要更多测试）
...
- pkg/scenes/game_scene.go - 集成 RewardAnimationSystem
