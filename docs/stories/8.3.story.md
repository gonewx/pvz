...
## Status
In Progress - Phase 2 core implementation completed (Task 7-11 ✅, Task 12 ✅), partial Task 13 ✅, remaining tasks: unit tests, manual testing, documentation
...
- [x] 创建 `pkg/components/reward_animation_component.go`：
  - [x] 定义 `RewardAnimationComponent`：
    ...
    ```
  - [x] 添加 GoDoc 注释
- [x] 创建 `pkg/components/reward_panel_component.go`：
...
- [x] 创建 `pkg/systems/reward_animation_system.go`：
  ...
  - [x] 实现 `TriggerReward(plantID string)` 方法：
    - [x] 创建奖励动画实体
    - [x] 添加 RewardAnimationComponent（Phase="dropping"）
    - [x] 添加 PositionComponent（起点：屏幕右侧上方）
    - [ ] 添加 SpriteComponent（卡片包图片）- TODO: 待资源集成
    - [ ] 触发 Award.xml 粒子特效（跟随卡片包移动）- TODO: 待粒子系统集成
  - [x] 实现 `Update(dt float64)` 方法：
    - [x] Phase "dropping": 抛物线运动逻辑
      - [x] 更新位置：`X += VelocityX * dt`, `Y += VelocityY * dt`
      - [x] 应用重力：`VelocityY += gravity * dt`
      - [x] 检测到达目标（草坪中央）→ 切换到 "bouncing"
    - [x] Phase "bouncing": 弹跳动画
      - [x] 播放 3 次小幅弹跳（使用 sin 曲线）
      - [x] 完成后切换到 "expanding"
    - [x] Phase "expanding": 卡片展开动画
      - [x] 等待玩家点击卡片包
      - [ ] 点击后触发展开特效（粒子爆发）- TODO: 待粒子系统集成
      - [x] 切换到 "showing"，显示奖励面板
    - [x] Phase "showing": 显示奖励面板
      - [x] 创建 RewardPanelComponent 实体
      - [x] 加载植物信息（名称、描述）
      - [x] 卡片渐渐放大并移动到上方中央（scale: 0.5 → 1.5）
      - [x] 等待玩家点击关闭
    - [x] Phase "closing": 关闭奖励面板
      - [x] 淡出动画
      - [x] 清理奖励实体
      - [ ] 触发场景切换（返回主菜单）- TODO: 待场景管理器集成
  - [x] 实现辅助方法：
    - [x] 使用物理公式实现抛物线运动
    - [x] `handleClick()` - 点击检测
  - [x] 添加 GoDoc 注释
...
5. **Task 15 改进** - 创建专用验证程序提升测试效率 ✅
   ...
   - **成果**:
     ...
     - ⏳ `cmd/verify_reward/` - 奖励动画验证程序（待创建）
       - 直接触发奖励动画，无需打完关卡
   - **使用示例**:
     ```bash
     # 测试 1-2 关卡开场动画
     ...
     # 调整镜头速度
     go run cmd/verify_opening/main.go --level=1-3 --speed=500
     ```
...
- **cmd/verify_opening/README.md** ✅ (验证程序文档)
