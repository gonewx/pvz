...
3. **粒子效果资源**：
   - ✅ `data/particles/Award.xml` - 完整光芒特效系统（12个发射器）
...
**资源文件**:
- `data/particles/Award.xml` - 奖励粒子效果配置
...
### 优化内容：扩展僵尸行转换系统 - 支持渐变和瞬间两种模式

**背景**：
- 用户需求：僵尸从非有效行进攻时，希望能够直接出现在目标行上，无需渐变动画
- 但渐变动画在未来特殊关卡仍需使用，因此需要**同时保留两种方案**

**实施变更**：

1. **扩展 ZombieTargetLaneComponent** (pkg/components/zombie_target_lane.go)
   - 新增 `LaneTransitionMode` 枚举类型
     - `TransitionModeGradual`: 渐变模式（3秒Y轴速度移动）
     - `TransitionModeInstant`: 瞬间模式（立即调整Y坐标）
   - 添加 `TransitionMode` 字段到组件

2. **重构 ZombieLaneTransitionSystem** (pkg/systems/zombie_lane_transition_system.go)
   - 修改 `Update()` 方法，添加模式分支处理
   - 新增 `handleInstantTransition()` 方法 - 瞬间调整逻辑
   - 新增 `handleGradualTransition()` 方法 - 渐变动画逻辑（保留原逻辑）

3. **扩展 LevelConfig** (pkg/config/level_config.go)
   - 添加 `LaneTransitionMode string` 字段
   - 支持配置值："instant"（瞬间）或 "gradual"（渐变）
   - 默认值："instant"（向后兼容）

4. **修改 WaveSpawnSystem** (pkg/systems/wave_spawn_system.go)
   - 修改 `addTargetLaneComponent()` 方法，从配置读取模式
   - 新增 `getLaneTransitionMode()` 方法 - 解析配置字符串为枚举
   - 简化VY速度计算逻辑（移至 ZombieLaneTransitionSystem）

5. **更新关卡配置** (data/levels/level-1-3.yaml)
   - 添加 `laneTransitionMode: "instant"` 字段

**向后兼容性**：
- ✅ 现有关卡未配置 `laneTransitionMode` 时，默认使用瞬间模式
- ✅ 渐变动画逻辑完全保留，未来关卡可配置使用
- ✅ 所有现有测试通过
...
```yaml
# 标准关卡（瞬间模式）
laneTransitionMode: "instant"  # 僵尸立即调整到有效行

# 特殊关卡（渐变模式）
laneTransitionMode: "gradual"  # 僵尸3秒平滑移动到有效行
```
