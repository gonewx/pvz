...
- [x] 手动测试游戏（已由用户验证通过）:
  - [x] 验证 AC 1: 植物卡片栏显示坚果墙卡片
  - [x] 验证 AC 1: 可以选择并种植坚果墙(消耗 50 阳光)
  - [x] 验证 AC 2: 坚果墙种植后保持静止,不发射子弹
  - [x] 验证 AC 3: 坚果墙能承受僵尸长时间啃食(远超向日葵/豌豆射手)
  - [x] 验证 AC 4: 生命值降低时,坚果墙外观出现裂痕变化
  - [x] 验证 AC 5: 生命值耗尽时,坚果墙消失
...
| 2025-10-12 | 2.0 | 完整实现坚果墙功能，修复3个关键bug，通过所有测试 | Dev Agent (James) |
...
- 代码格式化: `gofmt -w .` - 完成
- 最终编译: `go build -o pvz-final .` - 成功
- 功能验证: 游戏内测试通过，坚果墙外观切换正常工作
...
13. **Bug修复 - 外观状态切换优化**: 改进了handleWallnutBehavior()的状态切换逻辑。使用帧数判断当前状态（16帧=完好，11帧=轻伤，15帧=重伤），避免每帧重复加载相同动画。添加了详细的切换日志，记录生命值和状态变化，便于调试验证
14. **Bug修复 - 坚果墙行为未被调用**: 修复了坚果墙的handleWallnutBehavior()根本未被调用的严重bug。原因是BehaviorSystem的植物查询要求TimerComponent，但坚果墙作为纯防御植物不需要计时器。移除了plantEntityList查询中的TimerComponent要求，改为只查询BehaviorComponent+PlantComponent+PositionComponent，确保所有植物类型都能被正确处理
