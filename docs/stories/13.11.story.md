...
- [ ] 详细设计 `CalculateRootMotionDelta()` 函数逻辑
  - [ ] 定义函数签名和返回值（deltaX, deltaY, error）
  - [ ] 设计瞬移检测阈值（MAX_DELTA = 100.0 像素）
  - [ ] 设计空帧继承处理逻辑
- [ ] 设计 ReanimComponent 字段扩展
  - [ ] `LastGroundX float64` - 上一帧 _ground 轨道 X 坐标
  - [ ] `LastGroundY float64` - 上一帧 _ground 轨道 Y 坐标
- [ ] 设计后备方案策略
  - [ ] 根运动失败时回退到固定速度法
  - [ ] 记录警告日志

### Task 2: 实现根运动工具库 (AC: 1)
- [ ] 创建 `pkg/utils/root_motion.go` 文件
- [ ] 实现 `CalculateRootMotionDelta()` 函数
  - [ ] 验证参数（reanimComp 不为 nil）
  - [ ] 获取 `_ground` 轨道数据
  - [ ] 获取当前动画的物理帧索引
  - [ ] 计算当前帧与上一帧的位移增量
  - [ ] 实现瞬移检测逻辑（deltaX/deltaY > MAX_DELTA 时返回 0）
  - [ ] 更新 `LastGroundX/Y` 字段
  - [ ] 返回 (deltaX, deltaY, error)
- [ ] 实现 `getGroundPosition()` 辅助函数
  - [ ] 处理边界情况（frameIndex 越界）
  - [ ] 处理 Reanim 空帧继承机制
  - [ ] 返回 (x, y)
- [ ] 添加完整的 GoDoc 注释
  - [ ] 函数用途和工作原理
...
- [ ] 编辑 `pkg/components/reanim_component.go`
- [ ] 在 `ReanimComponent` 结构体中添加字段：
  ```go
...
- [ ] 编辑 `pkg/systems/behavior/zombie_behavior_handler.go`
- [ ] 定位 `handleZombieBasicBehavior()` 函数（约第 69-71 行）
- [ ] 修改位移计算逻辑：
  - [ ] 获取 ReanimComponent
  - [ ] 调用 `utils.CalculateRootMotionDelta(reanim, "_ground")`
  - [ ] 如果成功（err == nil）：
    - [ ] 应用位移增量：`position.X += deltaX`, `position.Y += deltaY`
    - [ ] 添加 DEBUG 日志（verbose 标志控制）
  - [ ] 如果失败（err != nil）：
    - [ ] 记录警告日志
    - [ ] 回退到固定速度法：`position.X += velocity.VX * deltaTime`
  - [ ] 如果没有 ReanimComponent：
    - [ ] 使用固定速度法（保持向后兼容）
- [ ] 更新注释说明根运动系统的使用

### Task 5: 初始化僵尸工厂 (AC: 4)
- [ ] 编辑 `pkg/entities/zombie_factory.go`
- [ ] 修改 `NewZombieEntity()` 函数
  - [ ] 在创建 `ReanimComponent` 时初始化：
    ```go
    LastGroundX: 0.0,
    LastGroundY: 0.0,
    ```
- [ ] 修改 `NewConeheadZombieEntity()` 函数
  - [ ] 添加相同的初始化代码
- [ ] 修改 `NewBucketheadZombieEntity()` 函数
  - [ ] 添加相同的初始化代码
- [ ] 添加注释说明初始化的重要性

### Task 6: 编写单元测试 (AC: 5)
- [ ] 创建 `pkg/utils/root_motion_test.go` 文件
- [ ] 实现测试用例 1: `TestCalculateRootMotionDelta_NormalMovement`
  - [ ] 创建测试用 ReanimComponent（预设 LastGroundX/Y）
  - [ ] 创建测试用 _ground 轨道（包含多个帧）
  - [ ] 执行 `CalculateRootMotionDelta()`
  - [ ] 验证 deltaX/deltaY 正确
  - [ ] 验证 LastGroundX/Y 已更新
- [ ] 实现测试用例 2: `TestCalculateRootMotionDelta_LoopReset`
  - [ ] 模拟动画循环（LastGroundX=200, 当前帧X=0）
  - [ ] 验证返回 deltaX=0（检测到瞬移）
- [ ] 实现测试用例 3: `TestCalculateRootMotionDelta_MissingTrack`
  - [ ] 传入不存在的轨道名称
  - [ ] 验证返回错误
  - [ ] 验证 deltaX/deltaY 为 0
- [ ] 实现测试用例 4: `TestGetGroundPosition_EmptyFrameInheritance`
  - [ ] 创建包含空帧的轨道
  - [ ] 验证空帧继承前一帧的坐标
- [ ] 运行测试：`go test -v pkg/utils/root_motion_test.go`
- [ ] 验证测试覆盖率 > 80%：`go test -cover pkg/utils`

### Task 7: 性能基准测试 (AC: 6)
- [ ] 创建 `pkg/utils/root_motion_bench_test.go` 文件
- [ ] 实现基准测试：`BenchmarkCalculateRootMotionDelta`
  - [ ] 准备测试数据（ReanimComponent + _ground 轨道）
  - [ ] 循环调用 `CalculateRootMotionDelta()`
  - [ ] 记录执行时间（ns/op）
- [ ] 实现对比基准：`BenchmarkFixedVelocityMethod`
  - [ ] 对比固定速度法的性能
- [ ] 运行基准测试：`go test -bench=. -benchmem pkg/utils`
- [ ] 验证性能差异 < 5%

### Task 8: 集成测试验证 (AC: 6)
- [ ] 编译游戏：`go build -o /tmp/pvz main.go`
- [ ] 运行关卡 1-1，验证：
  - [ ] 僵尸移动流畅
  - [ ] 僵尸脚步与地面同步（目视检查）
  - [ ] 动画循环平滑（观察 5+ 循环）
...
- [ ] 运行关卡 1-10（大量僵尸），验证：
  - [ ] 多僵尸同屏无性能下降
  - [ ] 所有僵尸类型（basic, conehead, buckethead）正常
- [ ] 检查日志：
  - [ ] 无根运动失败的警告
  - [ ] verbose 模式下能看到根运动 DEBUG 日志
