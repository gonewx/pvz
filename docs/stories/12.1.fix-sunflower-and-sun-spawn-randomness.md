...
- [ ] **Task 6: 手动测试和验证** (AC: 1-10)
  - [ ] 运行游戏：`go run . --verbose > /tmp/story-12.1-test.log 2>&1`
  - [ ] 种植3棵向日葵（不同位置），验证独立生产阳光
  - [ ] 观察首次生产（7秒）和后续生产（24秒）
  - [ ] 观察自动阳光生成至少5次，记录时间间隔和位置
  - [ ] 种植边缘位置的向日葵（左边缘、右边缘），验证阳光不超出屏幕
  - [ ] 截取关键时刻的游戏画面
  - [ ] 提取日志中的关键片段（阳光生成、随机偏移、边界检查）
  - [ ] 创建测试证据文档：`.ai/story-12.1-test-evidence.md`
...
- [x] **Task 8: 文档更新和提交**
  ...
  - [x] 审查所有修改，确保符合验收标准
  - [ ] 创建 Git commit：`git add . && git commit -m "fix(story-12.1): 修复向日葵阳光生产和增强随机性"`
  - [ ] 检查 commit 是否成功
...
- [ ] **验收标准验证**（待用户测试）
  - [ ] AC1-AC5: 所有功能需求通过手动测试
  - [ ] AC6: 现有功能无回归
  - [x] AC7: 日志输出清晰完整
  - [x] AC8: 单元测试通过（如果新增）：无新增测试，现有测试无回归
  - [ ] AC9: 手动测试完成，记录证据
  - [ ] AC10: 边界检查验证（种植边缘向日葵，确认阳光完整显示）

- [x] **质量检查**
  - [x] 所有测试通过（`go test ./...`）：无新回归
  - [x] 代码遵循项目编码标准
  - [ ] 提交消息清晰描述修复内容：待 Git commit
  - [x] 日志证据已记录到 `.ai/` 目录
...
**Task 6: 手动测试和验证**
- **待用户执行：** 代码修改已完成，需要用户运行游戏验证
- 测试步骤：
  1. 运行：`go run . --verbose > /tmp/story-12.1-test.log 2>&1`
  2. 种植3棵向日葵（不同位置），观察阳光生产
...
**待办事项：**
- **需要用户执行手动测试（Task 6）** - 验证所有功能需求和验收标准
- 如用户确认向日葵不生产阳光，需要提供运行时日志或录像证据
