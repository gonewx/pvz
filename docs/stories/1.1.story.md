...
4. 窗口标题应设置为"植物大战僵尸 - AI复刻版"。
5. 可以通过点击窗口的关闭按钮正常退出应用程序。

## Tasks / Subtasks
- [x] Task 1: 初始化 Go 项目 (AC: 1)
  - [x] 创建 `go.mod` 文件,设置模块路径为 `github.com/gonewx/pvz`
...
- [x] Task 4: 实现基础 Game 窗口 (AC: 3, 4, 5)
  ...
  - [x] 在 `main()` 函数中设置窗口标题为 "植物大战僵尸 - AI复刻版"
...
2. **模块路径**: 使用 `github.com/gonewx/pvz`
...
- 成功初始化Go模块,设置路径为`github.com/gonewx/pvz`
...
- **Given**: 新创建的项目
- **When**: 初始化Go模块
- **Then**: go.mod文件存在且模块路径为`github.com/gonewx/pvz`
- **验证**: ✓ go.mod文件正确配置
