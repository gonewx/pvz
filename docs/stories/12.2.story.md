...
- [x] **1.1: 设计 BottomButtonComponent 组件**
  - [x] 定义组件字段（ButtonType, NormalImage, HoverImage 等）
  - [x] 添加 GoDoc 注释
  - [x] 创建文件：`pkg/components/bottom_button_component.go`

- [x] **1.2: 定义底部按钮类型枚举**
  - [x] 创建 `BottomButtonType` 枚举（Options, Help, Quit）
  - [x] 定义资源 ID 映射关系
...
- [x] **2.1: 创建 BottomButton 实体工厂**
  - [x] 创建文件：`pkg/entities/bottom_button_factory.go`
  - [x] 实现函数：`NewBottomButtonEntity(em, rm, buttonType, x, y)`
  - [x] 添加组件：
...
- [x] **3.1: 定义按钮位置常量**
  - [x] 在 `pkg/config/ui_config.go` 中定义常量：
    ```go
    const (
        // 底部按钮布局
        BottomButtonStartX  = 555.0   // 第一个按钮 X 坐标（选项）
        BottomButtonY       = 555.0   // 所有按钮 Y 坐标（右下角）
        BottomButtonSpacingX = 10.0   // 按钮水平间距
    )
    ```

- [x] **3.2: 实现按钮位置计算函数**
  - [x] 创建函数：`func CalculateBottomButtonPosition(buttonIndex int) (x, y float64)`
  - [x] 返回第 N 个按钮的正确位置
...
- [x] **4.1: 修改 MainMenuScene 结构体**
  - [x] 删除旧字段：`buttons []components.Button`（如果还在使用）
  - [x] 添加新字段：
    ```go
    bottomButtons   []ecs.EntityID  // 3 个底部功能按钮实体
    ```

- [x] **4.2: 创建 initBottomButtons 方法**
  - [x] 删除旧的 `initButtons()` 方法中的退出按钮逻辑
  - [x] 创建新方法：`initBottomButtons()`
  - [x] 创建 3 个底部按钮实体：
    ```go
    func (m *MainMenuScene) initBottomButtons() {
        for i, btnType := range []BottomButtonType{BottomButtonOptions, BottomButtonHelp, BottomButtonQuit} {
            x, y := config.CalculateBottomButtonPosition(i)
...
- [x] **4.3: 实现按钮交互逻辑**
  - [x] 在 `Update()` 方法中添加底部按钮悬停检测
  - [x] 在 `Update()` 方法中添加底部按钮点击处理
  - [x] 实现点击回调函数：
    ```go
...
- [x] **5.2: 添加缺失的资源配置**
  - [x] 如果资源未配置，添加到 `assets/config/resources.yaml`
  - [x] 验证资源文件存在于 `assets/images/` 目录

### Task 6: 测试和验证 (AC: 7, 9)

- [x] **6.1: 编写单元测试**
  - [x] 测试文件：`pkg/config/ui_config_test.go`
  - [x] 测试用例：
    - `TestCalculateBottomButtonPosition` - 验证位置计算
    - `TestBottomButtonLayout` - 验证布局间距
    - `TestBottomButtonSpacing` - 验证按钮间距
    - `TestBottomButtonTotalWidth` - 验证总宽度
...
- [x] **7.1: 代码格式化**
  - [x] 运行 `gofmt -w pkg/components/bottom_button_component.go`
  - [x] 运行 `gofmt -w pkg/entities/bottom_button_factory.go`
  - [x] 运行 `gofmt -w pkg/scenes/main_menu_scene.go`
  - [x] 运行 `gofmt -w pkg/config/ui_config.go`
...
- `pkg/components/bottom_button_component.go` - 底部按钮组件定义
- `pkg/entities/bottom_button_factory.go` - 底部按钮实体工厂
- `pkg/config/ui_config.go` - UI 布局配置常量
- `pkg/config/ui_config_test.go` - UI 配置单元测试

**修改文件**:
- `pkg/scenes/main_menu_scene.go` - 主菜单场景逻辑（添加底部按钮）

**资源文件** (已存在，无需修改):
- `assets/config/resources.yaml` - 资源配置（底部按钮图片资源已配置）
- `assets/images/SelectorScreen_Options1.png` - 选项按钮正常态
- `assets/images/SelectorScreen_Options2.png` - 选项按钮悬停态
- `assets/images/SelectorScreen_Help1.png` - 帮助按钮正常态
- `assets/images/SelectorScreen_Help2.png` - 帮助按钮悬停态
- `assets/images/SelectorScreen_Quit1.png` - 退出按钮正常态
- `assets/images/SelectorScreen_Quit2.png` - 退出按钮悬停态

### Completion Notes

**实现完成**:
- ✅ 创建了 `BottomButtonComponent` 组件，定义了 3 种按钮类型（Options/Help/Quit）
- ✅ 实现了底部按钮实体工厂 `NewBottomButtonEntity`，支持自动加载资源
- ✅ 添加了 UI 配置文件 `ui_config.go`，定义了按钮布局常量和位置计算函数
- ✅ 重构了 `MainMenuScene`，集成了底部按钮逻辑（初始化、更新、绘制）
- ✅ 实现了按钮交互功能：悬停效果、点击回调、面板集成
- ✅ 编写了 4 个单元测试，验证布局计算逻辑，所有测试通过
- ✅ 完成代码格式化（gofmt）和编译检查

**待手工测试**:
- ⏳ 集成测试（需要运行游戏验证 3 个按钮显示和交互）
- ⏳ 视觉对比测试（需要与原版截图对比，微调按钮位置）

**技术亮点**:
- 使用 ECS 架构，遵循"零耦合"原则，底部按钮作为独立实体管理
- 通过 Story 12.3 的模块系统集成帮助和选项面板
- 按钮尺寸不同（Options: 81x31, Help: 48x22, Quit: 47x27），使用动态宽度计算
- 单元测试覆盖位置计算、间距验证、布局合法性检查
...
| Date | Version | Description | Author |
|------|---------|-------------|--------|
| 2025-11-16 | 1.0 | Story 12.2 实现完成（代码部分）| James (Dev Agent) |
