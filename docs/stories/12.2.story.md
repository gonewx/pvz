...
**As a** 玩家（游戏玩家）,
**I want** 在主菜单底部看到 3 个花瓶样式的功能按钮（选项/帮助/退出），并能通过点击访问这些功能,
**so that** 我能方便地调整游戏设置、查看帮助信息和退出游戏。
...
4. **功能实现：**
   - **选项按钮**: 打开设置对话框（音量、全屏等）- 依赖 Story 12.3
   - **帮助按钮**: 打开帮助对话框（游戏说明）- 依赖 Story 12.3
   - **退出按钮**: 退出游戏（已实现，需保留）

**技术挑战：**
1. 需要替换现有的退出按钮实现
2. 需要处理 2 个新按钮（选项、帮助）
3. 设置和帮助对话框暂时无法打开（依赖 Story 12.3）
4. 需要精确定位按钮在右下角
...
**AC3: 选项和帮助按钮临时实现**
- Given: Story 12.3（对话框系统）尚未实现
- When: 点击选项或帮助按钮
- Then:
  - 应播放点击音效
  - 应打印日志提示（如"选项对话框功能尚未���现"）
  - **不应崩溃或报错**
  ...
  - [ ] 实现点击回调函数：
    ...
    func (m *MainMenuScene) onBottomButtonClicked(btnType BottomButtonType) {
        ...
        case BottomButtonOptions:
            // TODO: Story 12.3 - 打开设置对话框
            log.Println("[MainMenu] Options button clicked - Dialog not yet implemented")

        case BottomButtonHelp:
            // TODO: Story 12.3 - 打开帮助对话框
            log.Println("[MainMenu] Help button clicked - Dialog not yet implemented")
...
- 本 Story 实现简单，主要是添加 3 个按钮并处理交互
- 与 Story 12.1 可以并行开发，但需要协调 MainMenuScene 的修改
- 选项和帮助按钮的完整功能依赖 Story 12.3（对话框系统）
- 建议先完成基础功能（显示和交互），Story 12.3 完成后再添加对话框
