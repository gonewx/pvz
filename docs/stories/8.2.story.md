...
- [ ] 修改 `pkg/systems/render_system.go` 或创建专门的 `TutorialRenderSystem`:
  ...
  - [ ] **绘制教学文本（根据原版截图）**：
    - [ ] **位置**: 屏幕底部中央（Y: screenHeight - 200）
    - [ ] **字体**: 白色文字 + 黄色描边（stroke）
    - [ ] **字体大小**: 28-32px（较大，易读）
    - [ ] **对齐**: 水平居中
    - [ ] **无背景框**（与原版一致）
  - [ ] **可选：向上箭头指示符**（指向植物卡片，后续优化）
  - [ ] 渲染层级：在UI之上，阳光之下
...
- [ ] 验证教学流程：
  - [ ] 游戏开始后无教学提示（等待玩家自然收集阳光）
  - [ ] 收集阳光后显示"点击拾取种子包！"提示（底部中央，白色文字+黄色描边）
  - [ ] 种植豌豆射手后显示"干得漂亮！"提示
  - [ ] 僵尸生成后显示"别让僵尸靠近你的房子！"提示
