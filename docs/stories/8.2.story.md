...
4. **更新 1-1 关卡配置** (`data/levels/level-1-1.yaml`)
   ...
   - **添加教学步骤配置（使用 LawnStrings.txt 文本键）**：
     ...
     tutorialSteps:
       - trigger: "gameStart"
         textKey: "ADVICE_CLICK_ON_SUN"  # 从 LawnStrings.txt 读取："点击收集掉落的阳光！"
         action: "waitForSunCollect"
       - trigger: "sunCollected"
         textKey: "ADVICE_CLICK_PEASHOOTER"  # "点击豌豆射手，再种一颗！"
         action: "waitForPlantPlaced"
       - trigger: "plantPlaced"
         textKey: "ADVICE_PLANTED_PEASHOOTER"  # "干得漂亮！"
         action: "waitForZombieKilled"
     ```
...
6. **教学流程验证**
   - 进入 1-1 关卡后，自动显示第一条教学文本
   - 完成教学要求后，自动进入下一步
   - 所有教学步骤完成后，系统自动禁用
   - 关卡结束后解锁豌豆射手（通过 PlantUnlockManager）
...
- [ ] 修改 `pkg/systems/render_system.go` 或创建专门的 `TutorialRenderSystem`:
  - [ ] 在 `Draw()` 方法中添加教学文本渲染逻辑
  - [ ] 查询 TutorialTextComponent 实体（使用泛型 API）
  - [ ] 绘制半透明背景框：
    - [ ] 位置：屏幕中上方（X: screenWidth/2, Y: 100）
    - [ ] 大小：根据文本长度动态调整（最小宽度400px，高度80px）
    - [ ] 背景色：黑色，透明度 0.7
  - [ ] 绘制教学文本：
    - [ ] 使用白色字体
    - [ ] 字体大小：24px
...
### Task 8: 关卡完成后解锁豌豆射手 (AC: 6)
- [ ] 修改 `pkg/systems/level_system.go` 或 `pkg/scenes/game_scene.go`:
  - [ ] 在关卡完成检测逻辑中（所有僵尸被击败）：
    - [ ] 检查 `levelConfig.ID == "1-1"`
    - [ ] 调用 `gs.GetPlantUnlockManager().UnlockPlant("peashooter")`
    - [ ] 显示"获得新植物"提示（可选，后续Story实现）
  - [ ] 添加日志输出：`log.Printf("Unlocked plant: peashooter")`
...
- [ ] 验证关卡完成：
  - [ ] 击败所有僵尸后关卡结束
  - [ ] 豌豆射手解锁（通过日志或 PlantUnlockManager 查询验证）
...
- ✅ `TutorialStep` 结构体已定义（Trigger, Text, Action）
...
**触发条件类型**:
- `"gameStart"`: 游戏开始时立即触发
- `"sunCollected"`: 玩家收集阳光时触发
- `"plantPlaced"`: 玩家种植植物时触发
- `"zombieKilled"`: 僵尸被击败时触发
