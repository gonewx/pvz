...
- [ ] 创建 `pkg/components/tutorial_component.go`:
  - [ ] 定义 `TutorialComponent` 结构体：
    ```go
...
- [ ] 创建 `pkg/components/tutorial_text_component.go`:
  - [ ] 定义 `TutorialTextComponent` 结构体：
    ```go
...
- [ ] 创建 `pkg/game/lawn_strings.go`:
  - [ ] 定义 `LawnStrings` 结构体：
    ...
    ```
  - [ ] 实现 `NewLawnStrings(filePath string) (*LawnStrings, error)`
    - [ ] 读取 `assets/properties/LawnStrings.txt`
    - [ ] 解析格式：`[KEY]\n文本\n`
    - [ ] 返回字符串映射表
  - [ ] 实现 `GetString(key string) string`
    - [ ] 根据键返回文本
    - [ ] 如果键不存在，返回 `"[" + key + "]"`（调试用）
  - [ ] 添加 GoDoc 注释
- [ ] 修改 `pkg/game/game_state.go`:
  - [ ] 添加 `LawnStrings *game.LawnStrings` 字段
  - [ ] 在初始化时加载 LawnStrings.txt
- [ ] 创建单元测试 `pkg/game/lawn_strings_test.go`:
  - [ ] `TestLawnStrings_Load`: 验证文件加载
  - [ ] `TestLawnStrings_GetString`: 验证文本获取
  - [ ] `TestLawnStrings_MissingKey`: 验证缺失键处理

### Task 4: 实现 TutorialSystem 核心逻辑 (AC: 2)
- [ ] 创建 `pkg/systems/tutorial_system.go`:
  - [ ] 定义 `TutorialSystem` 结构体：
    ```go
    ...
    ```
  - [ ] 实现 `NewTutorialSystem(em *ecs.EntityManager, gs *game.GameState, levelConfig *config.LevelConfig) *TutorialSystem`
    - [ ] 创建教学实体
    - [ ] 添加 `TutorialComponent`，复制 `levelConfig.TutorialSteps`
    - [ ] 设置 `IsActive = true`
    - [ ] 返回系统实例
  - [ ] 实现 `Update(dt float64)` 方法：
    - [ ] 查询 TutorialComponent 实体（使用泛型 API）
    - [ ] 如果 `!IsActive`，直接返回（教学已完成）
    - [ ] 获取当前步骤 `currentStep := tutorial.TutorialSteps[tutorial.CurrentStepIndex]`
    - [ ] 检查触发条件是否满足：
      - [ ] `"gameStart"`: 游戏开始时立即触发（在 NewTutorialSystem 中标记）
      - [ ] `"sunCollected"`: 检查 GameState 是否有阳光增加
      - [ ] `"plantPlaced"`: 检查是否有新的植物实体创建
      - [ ] `"zombieKilled"`: 检查僵尸是否被击杀
    - [ ] 如果触发条件满足：
      - [ ] **从 LawnStrings 获取文本**：`text := s.gameState.LawnStrings.GetString(currentStep.TextKey)`
      - [ ] 显示教学文本（创建或更新 TutorialTextComponent，使用从 LawnStrings 加载的文本）
      - [ ] 标记当前步骤已完成 `tutorial.CompletedSteps[currentStep.Trigger] = true`
      - [ ] 移动到下一步 `tutorial.CurrentStepIndex++`
      - [ ] 如果所有步骤完成，设置 `IsActive = false`
  - [ ] 实现辅助方法：
    - [ ] `checkTriggerCondition(trigger string) bool` - 检查触发条件
    - [ ] `showTutorialText(text string)` - 显示教学文本
...
- [ ] **选项 1（推荐）：简化实现 - 使用 TrueType 字体模拟**
  - [ ] 使用现有的 `ResourceManager.LoadFont("assets/fonts/briannetod.ttf", 28)`
  - [ ] 使用 `text.Draw()` 绘制白色文字
  - [ ] 手动绘制黄色描边（在文字周围偏移1-2像素绘制多次）
  - [ ] **优点**: 实现快速，利用现有系统
  - [ ] **缺点**: 与原版字体略有差异
  
- [ ] **选项 2（完整实现）：位图字体系统**
  - [ ] **创建 BitmapFont 结构体** (`pkg/utils/bitmap_font.go`):
    ```go
    ...
    ```
  - [ ] **实现字体加载器** (`LoadBitmapFont(imagePath, metaPath string)`):
    - [ ] 加载 PNG 图集 (`HouseofTerror28.png`)
    - [ ] 解析 TXT 元数据 (`HouseofTerror28.txt`)
      - 解析 CharList: `(' ','A','B',...)`
      - 解析 WidthList: `(288,17,16,...)`
      - 解析 RectList: `((2,55,288,39), (0,0,28,54),...)`
    - [ ] 构建字符 → 矩形映射表
  - [ ] **实现文本渲染方法** (`DrawText(screen *ebiten.Image, text string, x, y int, align string)`):
    ```go
...
- [ ] **修改 RenderSystem**:
  - [ ] 在 `Draw()` 方法中添加教学文本渲染逻辑
  - [ ] 查询 TutorialTextComponent 实体（使用泛型 API）
  - [ ] **绘制教学文本（根据原版截图）**：
    - [ ] **位置**: 屏幕底部中央
      - Y: `screenHeight - 100` (距底部 100px)
      - X: `screenWidth / 2`（水平居中）
    - [ ] **字体**: HouseofTerror28（已内置白色+黄色描边效果）
    - [ ] **字体大小**: 28px（实际字符高度 54px）
    - [ ] **对齐**: `"center"`（水平居中）
    - [ ] **无背景框**（与原版一致）
  - [ ] **可选：向上箭头指示符**（使用 `assets/particles/DownArrow.png`，后续优化）
  - [ ] 渲染层级：在UI之上，阳光之下

**推荐方案**：先实现**选项 1**（TrueType 模拟），如果视觉效果不满意再升级到**选项 2**（位图字体）。

### Task 6: 更新 1-1 关卡配置 (AC: 4)
- [ ] 编辑 `data/levels/level-1-1.yaml`:
  - [ ] 确认 `openingType: "tutorial"`
  - [ ] 确认 `enabledLanes: [3]`
  - [ ] 确认 `skipOpening: true`
  - [ ] 添加 `tutorialSteps` 配置（使用 LawnStrings.txt 文本键）：
    ```yaml
    tutorialSteps:
      - trigger: "gameStart"
  ...
  - [ ] 更新僵尸配置：
    - [ ] 只有1波僵尸
    - [ ] 2-3只普通僵尸
    - [ ] 出现时间：`time: 15, 25, 35`（间隔较长）

### Task 7: 与现有系统集成 (AC: 5)
- [ ] 修改 `pkg/scenes/game_scene.go`:
  - [ ] 在 `NewGameScene()` 中检查 `levelConfig.OpeningType`
  - [ ] 如果 `openingType == "tutorial"` 且 `len(levelConfig.TutorialSteps) > 0`：
    - [ ] 创建 TutorialSystem 实例
    - [ ] 添加到系统列表（在 InputSystem 之后，UISystem 之前）
  - [ ] 添加日志输出：`log.Printf("Tutorial system activated for level %s", levelConfig.ID)`
- [ ] 修改 `pkg/systems/input_system.go`（可选，如需限制操作）:
  - [ ] 添加方法 `IsTutorialActive() bool`
  - [ ] 在植物选择逻辑中检查教学状态
  - [ ] 如果教学未完成特定步骤，禁止某些操作
- [ ] 修改 `pkg/systems/wave_spawn_system.go`（可选）:
  - [ ] 检查 LevelConfig 是否为教学关卡
  - [ ] 教学关卡使用更慢的僵尸生成节奏（可通过配置实现）

### Task 8: 关卡完成后解锁向日葵 (AC: 6)
- [ ] 修改 `pkg/systems/level_system.go` 或 `pkg/scenes/game_scene.go`:
  - [ ] 在关卡完成检测逻辑中（所有僵尸被击败）：
    - [ ] 检查 `levelConfig.ID == "1-1"`
    - [ ] 调用 `gs.GetPlantUnlockManager().UnlockPlant("sunflower")`  // 解锁向日葵，非豌豆射手
    - [ ] 显示"获得新植物"提示（可选，后续Story实现）
  - [ ] 添加日志输出：`log.Printf("Unlocked plant: sunflower")`

### Task 9: 单元测试与集成测试 (AC: 6, 7)
- [ ] 创建 `pkg/systems/tutorial_system_test.go`:
...
- [ ] 创建 `pkg/config/level_config_tutorial_test.go`:
  - [ ] `TestLoadLevelConfig_TutorialSteps`: 验证教学步骤解析
  - [ ] `TestLevel1_1_TutorialConfig`: 验证 1-1 配置符合教学要求
- [ ] 运行测试：`go test ./pkg/systems -v -run TestTutorial`
- [ ] 运行测试：`go test ./pkg/config -v -run TestTutorial`
