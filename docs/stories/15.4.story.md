...
- [ ] **Task 4**: 创建 `render_reanim.go` 文件（AC: 4, 7）
  - [ ] 4.1: 在 `pkg/systems/` 目录下创建 `render_reanim.go`
  - [ ] 4.2: 添加文件头注释和 `package systems` 声明
  - [ ] 4.3: 添加必要的 import 声明

- [ ] **Task 5**: 移动 Reanim 渲染方法到 `render_reanim.go`（AC: 5）
  - [ ] 5.1: 移动 `renderReanimEntity` 方法（核心 Reanim 渲染逻辑，约 185 行）
  - [ ] 5.2: 移动 `getStemOffset` 方法（约 48 行）
  - [ ] 5.3: 移动 `findPhysicalFrameIndex` 方法（约 32 行）
  - [ ] 5.4: 移动 `mapLogicalFrameToPhysical` 方法（约 36 行）
  - [ ] 5.5: 保持所有方法的接收者签名 `(s *RenderSystem)`

- [ ] **Task 6**: 验证 render_system.go 完整性（AC: 6, 8）
  - [ ] 6.1: 确认 `render_system.go` 保留核心渲染方法
  - [ ] 6.2: 检查 `render_system.go` 文件行数（目标约 800-900 行）
  - [ ] 6.3: 检查 `render_reanim.go` 文件行数（目标约 200-300 行）
  - [ ] 6.4: 确认所有函数调用正常（无断链）

- [ ] **Task 7**: 编译和语法检查（AC: 9）
  - [ ] 7.1: 运行 `go build ./pkg/systems/...` 确保编译通过
  - [ ] 7.2: 运行 `gofmt -l pkg/systems/` 检查格式
  - [ ] 7.3: 运行 `golangci-lint run pkg/systems/` 检查代码质量（如果可用）

- [ ] **Task 8**: 单元测试验证（AC: 10）
  - [ ] 8.1: 运行 `go test ./pkg/systems/... -v`
  - [ ] 8.2: 确保所有测试通过（或测试失败是预先存在的问题）
  - [ ] 8.3: 运行完整项目测试：`go test ./pkg/... -v`

- [ ] **Task 9**: 手动测试（AC: 11）
  - [ ] 9.1: 启动游戏：`go run main.go --verbose`
  - [ ] 9.2: 进入关卡 1-1，测试粒子效果（樱桃炸弹爆炸、阳光收集、僵尸死亡）
  - [ ] 9.3: 测试 Reanim 动画渲染（植物动画、僵尸动画）
  - [ ] 9.4: 测试主菜单 Reanim 动画（SelectorScreen, SodRoll）
  - [ ] 9.5: 观察日志输出，确保无异常错误

- [ ] **Task 10**: 提交更改到版本控制
  - [ ] 10.1: 使用 `git add` 添加新文件和修改的文件
  ...
  - [ ] 10.3: Commit message 格式：
    ...
    - 创建 pkg/systems/particle_emitter.go (约 500 行)
    - 创建 pkg/systems/render_reanim.go (约 300 行)
    - particle_system.go 从 1233 行减少到约 700 行
    - render_system.go 从 1169 行减少到约 900 行
    - 所有测试通过，核心功能无破坏
    ```

- [ ] **文档更新**
  - [ ] 更新 `docs/architecture/unified-project-structure.md`（如果需要）：
    - 说明 `particle_emitter.go` 和 `render_reanim.go` 的用途
  - [ ] 更新 `docs/architecture/core-systems.md`（如果需要）：
    - 更新 ParticleSystem 和 RenderSystem 的文件组织说明
