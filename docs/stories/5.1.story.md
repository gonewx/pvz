...
|------|---------|-------------|--------|
| 2025-10-27 | 1.4 | Bug Fix: 植物死亡时释放网格占用，允许重新种植 | James (Dev) |
...
10. **BUG修复记录**
   ...
   - **修复3**: 植物死亡后网格占用未释放（2025-10-27）
     - 原因：植物死亡时仅删除实体，未调用 `LawnGridSystem.ReleaseCell()` 释放网格占用
     - 表现：僵尸吃掉植物后，玩家无法在该位置重新种植植物
     - 修复：在 `handleZombieEatingBehavior()` 中植物死亡时添加网格释放逻辑
     - 相关代码：
       - `pkg/systems/behavior_system.go:934-942` - 添加网格释放调用
       - `pkg/scenes/game_scene.go:318-325` - 传入 LawnGridSystem 依赖
       - `pkg/systems/behavior_system_grid_release_test.go` - 新增测试验证修复
...
- **Modified:**
  ...
  - `pkg/systems/behavior_system.go` - 实现啃食逻辑和状态机（Bug Fix: 添加网格释放）
  - `pkg/systems/behavior_system_test.go` - 添加7个新测试函数 + 测试辅助函数
  - `pkg/systems/behavior_system_grid_release_test.go` - Bug Fix: 添加网格释放测试
  - `pkg/scenes/game_scene.go` - Bug Fix: 传入 LawnGridSystem 依赖
  - `pkg/utils/zombie_resources.go` - 添加动画加载辅助函数
  - `docs/stories/5.1.story.md` - 更新任务进度和完成记录（Bug Fix 记录）
