...
**本 Story 的迁移范围**：
- ✅ **包含**：游戏世界实体（植物、僵尸、子弹、阳光、特效）
- ❌ **不包含**：UI 元素（植物卡片）

**组件使用策略**：
- 游戏世界实体 → `ReanimComponent`（支持复杂多部件动画）
- UI 元素 → 继续使用 `SpriteComponent`（简单高效）

**这是有意的架构设计，不是遗留问题**：
1. UI 元素由专门的渲染系统处理（`PlantCardRenderSystem`）
2. UI 不需要 Reanim 的复杂动画功能
3. 保持 UI 渲染的简单性和性能
4. QA 评审已确认这是合理的设计决策（参见 QA Results）

**特殊情况**：
- 植物预览实体：虽然是 UI 相关，但使用 `ReanimComponent`
  - 原因：预览需要显示植物的完整动画效果
  - 实现：通过 `InputSystem.createPlantPreview()` 直接创建
  - 注意：`NewPlantPreviewEntity` 工厂函数未被使用（已在后续清理中删除）
