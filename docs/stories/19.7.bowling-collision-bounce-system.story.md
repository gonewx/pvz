...
- [x] 修改 `pkg/config/layout_config.go`
  - [x] ~~`BowlingNutDamage = 1800`~~ (已移除，改为移除护甲或秒杀机制)
...
**伤害处理逻辑**（与樱桃炸弹不同）：
- 普通碰撞：有护甲则移除护甲（打掉帽子/桶），无护甲则秒杀
- 路障僵尸需要 2 次碰撞：第一次打掉帽子，第二次秒杀
- 铁桶僵尸需要 2 次碰撞：第一次打掉桶，第二次秒杀
- 爆炸坚果：范围内所有僵尸直接秒杀（使用 ExplosiveNutDamage = 1800）
...
**保龄球坚果特殊处理**：
普通碰撞采用"移除护甲或秒杀"机制，与樱桃炸弹的固定伤害不同：
