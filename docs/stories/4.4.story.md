...
- [x] **Task 6: 为 BehaviorSystem 死亡逻辑编写单元测试** (AC: 3, 4)
  - [ ] 更新 `pkg/systems/behavior_system_test.go`
...
- [x] **Task 7: 集成测试和验收标准验证**
  - [ ] 运行所有单元测试：`go test ./pkg/...`
  - [ ] 确保测试覆盖率 ≥ 80%（针对新增代码）
  - [ ] 运行游戏：`go run .`
  - [ ] 验证 AC 1：每个僵尸有独立的生命值（通过 HealthComponent）
  - [ ] 验证 AC 2：豌豆子弹击中僵尸时生命值减少 20
  - [ ] 验证 AC 3：僵尸生命值 ≤ 0 时死亡（触发死亡状态）
  - [ ] 验证 AC 4：僵尸死亡时播放死亡动画，动画完成后消失
  - [ ] 验证 AC 5（可选）：击中僵尸时播放音效（如果实现）

- [x] **Task 8: 代码质量检查**
  - [ ] 运行 `gofmt -w .` 格式化所有代码
  - [ ] 运行 `golangci-lint run`（如果可用）
  - [ ] 检查所有公开方法都有 GoDoc 注释
  - [ ] 检查所有错误都被正确处理
  - [ ] 检查所有魔法数字都定义为常量
  - [ ] 验证 PhysicsSystem 和 BehaviorSystem 遵循零耦合原则
