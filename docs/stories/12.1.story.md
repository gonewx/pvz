...
**当前进度**: Task 1-9 已完成 ✅ | 待性能测试和 QA 验证 ⏳
...
**AC2: 关卡进度正确显示** ⭐ (Nice-to-Have - 延后至后续迭代)
- Given: 玩家已完成关卡 1-4
- When: 主菜单加载完成
- Then:
  - 冒险模式按钮下方**可选**显示"LEVEL 1-4"
  - 数字使用精灵图渲染（`SelectorScreen_LevelNumbers.png`）
  - 数字居中对齐
- **注**: Task 6 延后至后续优化迭代，需要精灵图字体系统支持。当前版本不实现此功能。
...
- [ ] **5.1: 加载 highlight 图片资源**
  - [ ] 在 `NewSelectorScreenEntity()` 中加载所有 `_Highlight` 后缀图片
  - [ ] 资源列表：
    ```
    ...
    ```
  - [ ] 预加载到 `PartImages` 映射中（避免运行时加载导致闪烁）

- [ ] **5.2: 实现 updateButtonHighlight 方法**
  - [ ] 在 `MainMenuScene.Update()` 中调用
  - [ ] 检测鼠标悬停在哪个按钮上（复用 Task 4 的点击检测逻辑）
  - [ ] 切换图片资源：
    ```go
    // 伪代码
...
- [ ] **5.3: 实现音效播放**
  - [ ] 加载音效：`SOUND_GRAVEBUTTON`（石块摩擦音效）
  - [ ] 仅在鼠标进入按钮时播放一次（避免重复播放）
  - [ ] 鼠标离开时不播放音效
