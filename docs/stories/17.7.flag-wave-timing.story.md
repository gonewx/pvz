...
- [x] **Task 1: 扩展 WaveTimerComponent 组件** (AC: 1, 2, 3)
  - [x] 在 `pkg/components/wave_timer.go` 添加旗帜波字段:
    ...
    - `FinalWaveTextActive bool` - 白字是否激活
    - `FinalWaveTextTimeCs int` - 白字已显示时间（厘秒）
...
- [x] **Task 4: 实现最终波白字逻辑** (AC: 3)
  - [x] 在 `WaveTimingSystem` 中添加最终波处理:
    ...
    - 白字显示 500cs 后检查胜利条件
  - [x] 添加常量:
    - `FinalWaveTextDurationCs = 500` // 白字显示时间
...
- [x] **Task 6: 创建最终波白字 UI** (AC: 4)
  - [x] 扩展或创建最终波文字渲染逻辑:
    - 使用现有 `FinalWaveWarningComponent` 或创建新组件
    - 显示 "FINAL WAVE" 白字
    - 实现原版动画效果（从大到小、淡入淡出）

- [x] **Task 7: UI 渲染集成** (AC: 4)
  - [x] 在 `pkg/scenes/game_scene_ui.go` 中:
    - 添加红字警告渲染方法 `drawHugeWaveWarning()`
    - 添加最终波白字渲染方法 `drawFinalWaveText()`
    - 使用原版字体（DwarvenTodcraft 或 HouseofTerror）
    - 实现文本居中、缩放、闪烁效果

- [x] **Task 8: 加速刷新逻辑（旗帜波前）** (AC: 1)
...
- [x] **Task 9: 与 LevelSystem 集成** (AC: 1, 2, 3)
...
- 实现了最终波白字逻辑（5500cs 延迟，500cs 显示）
- 创建了 FlagWaveWarningComponent 和 FlagWaveWarningSystem
- 创建了 FinalWaveTextComponent 和 FinalWaveTextSystem
- 集成了 UI 渲染（drawHugeWaveWarning, drawFinalWaveText）
...
- `pkg/components/final_wave_text_component.go` - 最终波白字组件
- `pkg/systems/flag_wave_warning_system.go` - 红字警告系统
- `pkg/systems/final_wave_text_system.go` - 最终波白字系统
- `pkg/systems/flag_wave_warning_system_test.go` - 红字/白字系统测试
...
- `pkg/systems/level_system.go` - 集成旗帜波警告和最终波系统
...
**亮点**:
1. **组件设计规范**: `FlagWaveWarningComponent` 和 `FinalWaveTextComponent` 严格遵循 ECS 纯数据原则，不包含任何逻辑方法
2. **系统职责清晰**: 各系统（`FlagWaveWarningSystem`、`FinalWaveTextSystem`、`WaveTimingSystem`）职责单一，通过组件数据通信
...
| 3 | 白字显示 500cs | `ActivateFinalWaveText()` | `TestWaveTimingSystem_FinalWaveTextActivation` |
| 4 | 红字 UI 渲染 | `drawHugeWaveWarning()` | 运行时验证 |
| 4 | 白字 UI 渲染 | `drawFinalWaveText()` | 运行时验证 |
