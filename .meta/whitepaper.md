...
<img src="https://static.wikia.nocookie.net/plantsvszombies/images/c/c3/MainMenu.jpg" alt="Main Menu" width="500" />
...
### 2. 核心功能区（墓碑菜单）
游戏的主要模式通过从地底升起的墓碑按钮来选择。
*   **冒险模式 (Adventure)**：最大的墓碑，位于上方。字体最大，最显眼。
*   **迷你游戏 (Mini-Games)**：左下墓碑。需通关冒险模式3-2解锁，未解锁时呈灰色石块状。
*   **益智模式 (Puzzle)**：中下墓碑。需通关冒险模式解锁，包含“砸罐子”和“我是僵尸”。
*   **生存模式 (Survival)**：右下墓碑。需通关冒险模式解锁。
*   **交互动画**：鼠标悬停在墓碑上时，墓碑上的文字会高亮（中文版通常变为亮绿色或黄色），并伴有石块摩擦音效。

### 3. 用户管理与年度版特色 (Zombatar)
*   **用户栏 (左上角)**：
    *   显示形如木牌的UI，写着“欢迎你，[用户名]”。
    *   点击可进入用户管理界面，可以**新建**、**重命名**或**删除**存档。每个用户拥有独立的进度。
*   **僵尸化身 (Zombatar) - 年度版专属**：
    *   位于用户栏下方。
    *   允许玩家自定义一个僵尸头像（发型、胡须、肤色、眼镜、衣着等）。
    *   自定义的僵尸会出现在主菜单左侧，并且在游戏中替代**旗帜僵尸**带队冲锋。

### 4. 底部功能栏
当鼠标移动到屏幕底部时，以下图标会像玩偶匣一样弹起：
*   **选项 (Options)**：紫色花朵形状的齿轮。点击弹出设置菜单（音乐/音效音量、全屏、3D加速）。
*   **帮助 (Help)**：问号标志的木牌（部分版本整合在选项中）。
*   **退出 (Quit)**：右下角的一个紫色花瓶。点击后弹出确认框，确认后游戏关闭。
