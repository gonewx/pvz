...
### 2. 核心功能区（墓碑菜单）
游戏的主要模式通过从地底升起的墓碑按钮来选择。四个菜单项的图片默认墓碑动画已经包括并显示，只需要考虑交互时的高亮表现（通过使用后缀为 _Highlight 的同名图片替换），以及未解锁时的灰色表现即可（使用图片名中间有 `_shadow_` 的同名半透明图片覆盖）。

*   **冒险模式 (Adventure)**：墓碑最上面的菜单项，位于最上方。

    - 当没有选择用户或是新用户时，菜单要显示 SelectorScreen_StartAdventure_button。 
    - 非新用户时，要显示 SelectorScreen_Adventure_button, 菜单项下面会显示选择用户当前的关卡名称，使用下面的图片： assets/images/SelectorScreen_LevelNumbers.png （图片中是0-9的数字，用来组合关卡名，例如 : LEVEL 1-4）。

*   **玩玩小游戏 (Mini-Games)**：墓碑上第二个菜单项。需通关冒险模式3-2解锁，未解锁时呈灰色石块状。菜单要显示 SelectorScreen_Survival_button
*   **解谜模式 (Puzzle)**：墓碑上第三个菜单项。需通关冒险模式解锁，包含“砸罐子”和“我是僵尸”。
菜单要显示 SelectorScreen_Challenges_button
*   **生存模式 (Survival)**：墓碑上第四个菜单项。需通关冒险模式解锁。菜单要显示 SelectorScreen_Vasebreaker_button

*   **交互动画**：鼠标悬停在墓碑上的菜单时，如果菜单当前是解锁状态，墓碑上的菜单高亮（通过使用后缀为 _Highlight 的同名图片替换），并伴有石块摩擦音效， 如果是非解锁状态， 则表现不变，菜单项呈灰色石块状（使用图片名中间有 `_shadow_` 的同名半透明图片覆盖）， 点击后弹出未解锁的提示框。

### 3. 用户管理与年度版特色 (Zombatar)
*   **用户栏 (左上角)**：
    使用 selectorscreen 中的 anim_sign 动画展现
    *   显示形如木牌的UI，标题写着“欢迎回来，我的朋友！”，下面是用户名。
    *   下面是可点击的部分，可进入用户管理界面，可以**新建**、**重命名**或**删除**存档。每个用户拥有独立的进度。鼠标悬停在 这部分时， 需要切换图片 SelectorScreen_WoodSign2_press
