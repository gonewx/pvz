...
实现说明参见： @.meta/levels/loading-scene-ui-analysis.md
...
**交互行为：**
- **悬停状态**：鼠标悬停在用户名区域的下方的`如果这不是你的存档，请点我`的木板区域时，切换为 `SelectorScreen_WoodSign2_press` 图片。鼠标悬停的效果是在  woodsign2 轨道对应的图片上,点击是打开用户管理面板的
- **点击行为**：打开用户管理对话框
...
|------|------|
| **标题区域** | 顶部显示"你叫啥？" |
| **用户列表区域** | • 按创建日期排序显示所有用户<br>• 当前用户项背景为绿色（选中状态）<br>• 列表末尾显示"（建立一位新用户）"选项(点击后弹出新建用户对话框)，列表项的文字要居中显示 |
| **操作按钮区** | 4个石板风格按钮（详见下表） |

列表有一定的区域,不是全宽显示的, 区域背景需要用深灰色填充.列表项要有鼠标悬停效果,文字正常是黄色,如果悬停时, 变为白色
...
4. **创建成功后**：
   ...
   - 左上角用户栏显示新建的用户名（提前将用户名渲染到木牌图片上，然后让 Reanim 系统正常播放动画，这样用户名会自然跟随木牌一起移动）
