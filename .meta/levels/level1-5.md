...
2.  **强制引导 (The Lock)**:
    ...
    *   **操作限制**: 此时游戏进入**强引导模式**。
        ...
        *   如果玩家长时间（如 5秒）不操作，出现浮动手指动画：指向右上角铲子 -> 拖动/点击 -> 指向豌豆射手。
        ...
        1. [ADVICE_CLICK_SHOVEL]
        点击拾取铲子！

        2. [ADVICE_CLICK_PLANT]
        点击移除一颗植物！

        3. [ADVICE_KEEP_DIGGING]
        一直挖吧，直到你的草坪上没有植物！
...
坚果数量和出现频率建议按“阶段 + 动态调节”的方式设计：固定传送带容量，基础刷新间隔随关卡进度逐步加快，并按权重控制普通坚果 / 爆炸坚果的比例，确保玩家既有持续资源，又始终有一点紧张感。下面是可以直接给开发落表实现的一版设计说明。  

## 设计目标与约束

- 关卡形态：1-5 是第一个传送带类关卡，玩法为“坚果保龄球”，玩家只通过传送带提供的坚果（含普通坚果和爆炸坚果）清理整关僵尸，通关奖励为土豆地雷。 
- 设计目标：  
  - 始终给玩家足够的坚果进行操作，不出现长时间“无球可打”的空窗。  
  - 同时避免传送带堆满，玩家随便乱丢也能轻松过关，要让“什么时候用爆炸坚果”成为决策点。  
  - 随僵尸密度增加适度提高坚果出现频率，保持节奏紧张但可控。  

## 传送带基础规则设计

- 传送带容量  
  - 固定容量：例如 10 格。  
  - 当传送带已满时，不再生成新的坚果，防止资源无限堆积。  
- 坚果种类  
  - 普通坚果保龄球（Wall-nut）：低伤害 / 多次击打，主资源。
  - 爆炸坚果（Explode-o-nut）：小范围高伤害，数量少，用于处理压力峰值。  
- 生成方式  
  - 基于计时器周期生成：每过一段时间触发一次“生成尝试”。  
  - 每次生成时按权重随机决定生成普通坚果或爆炸坚果。  
  - 若当前传送带已满，则跳过本次生成（不排队补发）。  

## 三阶段数量与频率设计

以整关长度约 3～4 分钟为例，分为“安全铺垫-中期紧张-终盘压制”三段，以下时间/间隔可作为配置参考（供你们调优，不是必须死值）：  

### 前期（关卡进度 0%～30%）

- 目标：给玩家“上手机会”，让其熟悉弹道和反弹机制，压力较低。  
- 初始发放：  
  - 开局即开始发放普通坚果，同时僵尸进场，避免开场空挡。  
- 刷新节奏：  
  - 基础间隔：每 3.0～3.5 秒尝试生成 1 个坚果。  
  - 类型比例：  
    - 普通坚果：约 90%。  
    - 爆炸坚果：约 10%（用于教学“这个红色的是大招”）。  
  - 传送带保有量目标：  
    - 常态保持 3～5 个坚果在带上，短时可跌到 1～2 个，但不低于 1 个超过 3 秒。  

### 中期（关卡进度 30%～70%）

- 目标：僵尸数量增多，开始出现多行同时推进，需要玩家利用反弹一次打多只，开始真实考验决策。  
- 刷新节奏：  
  - 基础间隔：缩短到每 2.2～2.5 秒尝试生成 1 个坚果。  
  - 类型比例：  
    - 普通坚果：约 80%。  
    - 爆炸坚果：约 20%，保证场上始终有少量“解场牌”，但不会泛滥。  
  - 传送带保有量目标：  
    - 常态 4～6 个，避免玩家因为太满而随手乱丢；  
    - 若玩家连续 5 秒未使用任何坚果且已接近满带，可临时降一档刷新频率（例如间隔 +0.5 秒），引导其消耗存量。  

### 终盘（关卡进度 70%～100%，含最终大波）

- 目标：大波僵尸同时出现，节奏显著加快，给玩家“资源爆发感”，让其密集使用坚果清场。［］  
- 刷新节奏：  
  - 基础间隔：加快到每 1.5～1.8 秒尝试生成 1 个坚果。  
  - 类型比例：  
    - 普通坚果：约 70%。  
    - 爆炸坚果：约 30%，确保在大波期间玩家至少能拿到 3～5 个爆炸坚果。  
- 安全兜底：  
  - 当检测到“画面上同一行有 2 只以上僵尸接近安全线”（例如距离家门口剩余 3～4 格）时，如果过去 5 秒内未生成爆炸坚果，则强制把下一次生成的坚果类型设为爆炸坚果一次（随后恢复正常权重）。  

## 动态调节与极端情况处理

- 防止“无坚果可用”的极端情况：  
  - 若传送带在任意时刻连续 3 秒为空，则立即触发一次额外生成（必出普通坚果），用于补资源。  
- 防止“传送带堆满玩家却不丢”的极端情况：  
  - 若传送带连续 8 秒处于满状态，自动在接下来 5 秒内将刷新间隔提高 50%，直到带上格子低于满容量 2 格为止。  
- 难度可配置：  
  - 所有间隔、权重、保底逻辑建议通过配置表驱动，便于后续在“坚果保龄球 2”或高难度模式中微调，例如：总体提高僵尸密度但不同比例提高爆炸坚果生成率。

## 开发实现要点（给工程的落地建议）

- 用一个“关卡进度”的变量来驱动阶段切换，按阶段读取不同的刷新间隔和权重。  
- 传送带作为一个固定长度队列结构（例如 slice + head/tail 下标），生成逻辑只在队列未满时入队。  
- 刷新间隔、类型权重、保底规则全部放在数据表（如 YAML）中，关卡策划可自行调参，无需改代码。  
