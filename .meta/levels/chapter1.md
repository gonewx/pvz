...
- 背景图渲染（两阶段渲染）：使用配置中的背景图，延续上一关场景。采用两阶段渲染技术实现平滑过渡：
  - **初始状态**：开场动画期间：只显示底层背景有第3行草皮（IMAGE_SOD1ROW），延续1-1关卡的场景
  - **动画播放**：铺草皮动画开始后：渐进显示叠加层（第2,4行草皮通过动画展开），第2行和第4行同时播放草皮卷动画，叠加层渐进显示 IMAGE_SOD3ROW（3行整体草皮），逐步覆盖底层的单行草皮
  - **技术要点**：
...
*   **关卡流程与细节**: 撑杆僵尸会跳过他遇到的第一个植物。玩家需要学会用坚果墙挡在他面前让他提前跳跃，或者干脆不在最前一格放植物。寒冰射手的减速效果能有效克制他。

还需要实现：土豆地雷是一种爆炸植物 。种植后，土豆地雷需要 15 秒（大约相当于普通僵尸或其他行动缓慢的僵尸行走三格的时间）进行准备，而且不能种植在睡莲叶上，之后才会长出地面。在此期间，土豆地雷很容易受到攻击。准备就绪后，当敌人接触到它时，它会在 1x1（1 格）的范围内爆炸，对所有受影响的僵尸造成 1800 点伤害，并播放“土豆爆炸”的动画。用土豆地雷击杀僵尸将获得“ 土豆炸弹！” 成就 。
...
| **10** | basic x 5 , flag zombie,    | None | 4 extra points  |
...
| **20** | basic x 7 , flag zombie, Conehead Zombie, Pole Vaulting Zombie | None | Final Wave 5 extra points |

### **关-卡 1-8**
*   **场地布局**: 完整的5行草地。
*   **波数/旗帜**: **共1面旗帜**。
*   **解锁植物**: **双发射手 (Repeater)** (即双发射手)。
...
| **4**  | None Guaranteed
| **5**  | None Guaranteed   | None | 2 extra points |
| **6**  | Buckethead Zombie   | None |      |
| **7**  | basic       | None | 2 extra points |
| **8**  | basic       | None | 2 extra points |
| **9**  | basic       | None | 2 extra points |
| **10** | basic x 4, flag zombie, Conehead, Buckethead Zombie | None | Final Wave     |
...
*   **新流程 (1-8 及以后)**:
    关卡加载 -> 镜头右移预览僵尸 ->  **触发选卡** -> **进入选卡界面** -> 玩家点击 `一起摇滚吧!` 按钮 -> 镜头左移回草坪 -> **显示“Ready Set Plant”** -> 游戏开始。

**选卡界面 UI 交互规范 (UI Specs)**

- **界面布局**
*   **顶部卡槽 (Seed Slots)**: 在左上角显示空的植物选择栏。
*   **植物卡片仓库 (Inventory)**: 在卡槽下方显示所有已解锁的 7 张植物卡片。
    *   Peashooter, Sunflower, Cherry Bomb, Wall-nut, Potato Mine, Snow Pea, Chomper.
*   **开始按钮**: "一起摇滚吧!" 。初始状态为**不可点击**。

- **交互逻辑**
1.  **选择植物**:
    *   点击仓库里的卡片 -> 卡片飞入顶部卡槽的第一个空位 -> 仓库里该卡片变灰。
    *   音效: `seed_select.wav` (清脆的选中声)。
2.  **取消选择**:
    *   点击顶部卡槽里的卡片 -> 卡片飞回仓库 -> 仓库里该卡片恢复高亮。
    *   音效: `seed_deselect.wav`。
3.  **卡槽已满**:
    *   当选满 6 张时，仓库里剩下的第 7 张卡片无法点击
    *   **开始按钮**激活，变为可点击状态，并播放“按钮准备好”的音效。
4.  **鼠标悬停**： 鼠标悬停在植物卡包上时，有tip显示，样式和植物选择栏的tip一样，但是第一行是植物名称 ，第二行是详细描述
...
- 解锁预告信的表现和奖励动画和弹出铲子的效果一样，包括弹出(不是弹出卡包，资源是`ZombieNoteSmall.png`)、鼠标样式、箭头和后面的光晕效果。点击后，没有移动的动画，直接消失，并播放`Starburst`粒子效果。然后画面淡出，再淡入预告信的奖励面板画面。预告信的奖励面板和前面关卡的奖励面板ui界面不同，要和主菜单界面中帮助按钮点击后的界面效果一样，草坪背景，显示`ZombieNote.jpg`
