...
@.meta/screenshot/menu/menu1.png
...
*   **冒险模式 (Adventure)**：墓碑最上面的菜单，位于最上方。菜单项下面会显示选择用户当前的关卡名称，使用下面的图片： assets/images/SelectorScreen_LevelNumbers.png （图片中是0-9的数字，用来组合关卡名，例如 : LEVEL 1-4）

*   **玩玩小游戏 (Mini-Games)**：左下墓碑。需通关冒险模式3-2解锁，未解锁时呈灰色石块状。
...
*   **用户栏 (左上角)**：
    ...
    *   下面是可点击的部分，可进入用户管理界面，可以**新建**、**重命名**或**删除**存档。每个用户拥有独立的进度。
