...
- [x] 僵尸炸死后， 上一行僵尸突然消失，然后又从普通僵尸变成了路障僵尸?
...
- [x] level 1-5 ：直接测试这个关卡，背景是正确的，但正常游戏，从level 1-4 完成后，点下一关进入后，背景渲染的就是只有三行草皮的错误的背景，请修正。
- [x] 僵尸被打掉一只手后，进入啃食状态后，又会变成2只手，请修正(并不是必现). 有时僵尸被攻击后，会直接由2只手进入掉头后死亡状态（手没掉），有时又是正常的，先掉一只手，又掉头。
- [x] 不同种类僵尸的啃食速度怎么不同，配置中我们配置的速度是一样的
...
- [x] 右上角暂停菜单按钮要在开场动画（包括除草车动画\安放植物动画）全部完成后才显示， 另外植物选择栏的出现要添加从上向下滑入的动画效果，类似于 level 1-5 中传送带的入场效果

- [x]  土豆地雷的动画要实现的是，当僵尸靠近时，距离越近，头顶的警告灯闪烁的频率越快。无警告状态时是红灯亮和灭时长一样, 有警告时, 是红灯亮的时长越来越长.请分析实际的动画定义文件, 找到最优雅的实现方案.

  - anim_armed 土豆地雷的呼吸动画, 身体有上下抖动的感觉. 头顶的警告灯显示为灰色
  - anim_light 头顶的灰色警告灯
  - anim_glow 静态的头顶的警告灯红色的效果动画

- [x] 土豆地雷爆炸后只播放僵尸消失和爆炸粒子效果， 僵尸头和手掉落、变焦的动画不需要。

- [x] 现在植物和僵尸的渲染，在很多地方进行了偏移量的计算, 但初始实现时由于没有考虑僵尸的装备或者护甲，所以导致计算逻辑凌乱，甚至有很多写死的逻辑。请分析植物和僵尸渲染的场景，找出最优雅最合理的渲染的方式。

- [x] 撑杆僵尸的 run 动画速度和实际位移速度需要解耦——原版效果是动画播放很快（腿跑得快），但实际移动距离较小.

- [x] 撑杆僵尸跳跃前后要处理脚下的阴影 , 起跳时不显示脚下阴影 ,落地后再显示

- [x] /BMad:agents:architect 存档恢复后,僵尸的波次管理,也就是下一波僵尸在level 1-5 这种特殊关卡和其他关卡总是会互相影响, 要不就是level 1-5 僵尸在教学关卡就出现了,要不就是其他关卡,存档恢复后,不出僵尸了.总评估是不是我们的架构不合理? 

- [x] 奖励动画中卡包弹出后, 界面中其他元素都是可操作状态的.  只有点击卡包后,才进入奖励动画下一步.  奖励面板中右上角有些关卡是需要显示主菜单按钮的, 鼠标悬浮时要切换为手形，显示发光的效果，点击后跳转到主菜单界面。可以在关卡配置文件中指定.  按钮资源:
  - SeedChooser_Button2.png
  - SeedChooser_Button2_Glow.png

- [x] 撑杆僵尸的造型与普通僵尸差异比较大，在表现僵尸胜利的动画过程中会渲染出不合常理的状态。撑杆僵尸进入房子的动画效果要特殊处理。就是在撑杆僵尸触发了进入房子的第一阶段条件时， 镜头左移时。僵尸直接以行走状态绘制在第2阶段的第2个目标位置，然后同步展示僵尸走入房子的动画。
  - **变更提案已批准** (2025-12-19): Story 8.16 已创建，详见 `docs/sprint-change-proposals/2025-12-19-polevaulter-zombieswon-special-handling.md`
## 5.3
...
- [x] 铲子卡槽要和植物选择栏上对齐
- [x] 现在选择了植物后，铲子还可以重复选择，显示出重复的样子

- [x] 子弹也要有阴影的效果
- [x] 僵尸死亡后，只有第一发炮弹还有击中的效果
- [x] 土豆地雷在地下时，僵尸没有啃食
- [x] 植物种植时，预览在已经有植物的网格，不需要在网格中显示预览图像
- [x] 撑杆僵尸在跳起后，如果被炸死，是不需要显示炸焦的效果的，直接消失
- [x] 游戏中时是要播放背景音乐的
- [x] 移动端拖动铲子没有释放， 有重复的铲子音效
- [x] 樱桃炸弹爆炸时，本来是3x3网格范围内的僵尸，但是现在会炸到隔2行的撑杆僵尸。
- [x] 跃期间植物不应该向该僵尸射击，检测僵尸时跳过正在跳跃的撑杆僵尸

- [] 起跳后的僵尸是不会被子弹击中的,请修正

- [] 移动端拖动植物时偶尔会卡住，具体原因不明
